...
Route::prefix('/api/tools/race-master')->middleware(['auth', 'role:admin|eo'])->group(function () {
    ...
    Route::post('/sessions/{session}/update-timing', [App\Http\Controllers\Tools\RaceMasterApiController::class, 'updateSessionTiming'])->name('tools.race-master.api.sessions.update-timing');
...
Route::prefix('/api/tools/race-master/public')->group(function () {
    ...
    Route::post('/{slug}/update-timing', [App\Http\Controllers\Tools\RaceMasterApiController::class, 'publicUpdateSessionTiming'])->name('tools.race-master.api.public.update-timing');
...
Route::get('/api/events/upcoming', function () {
    $result = Illuminate\Support\Facades\Cache::remember('api.events.upcoming', 600, function () {
        try {
            ...
            $baseQuery = App\Models\Event::with('categories')
                ->select('id', 'name', 'slug', 'start_at', 'location_name', 'created_at', 'user_id', 'external_registration_link')
                ->published()
                ->where('start_at', '>=', $now)
...
Route::middleware('auth')->group(function () {
    ...
    Route::middleware('role:coach')->prefix('coach')->name('coach.')->group(function () {
        ...
        Route::post('/athletes/{enrollment}/update-vdot', [App\Http\Controllers\Coach\AthleteController::class, 'updateVdot'])->name('athletes.update-vdot');
        Route::post('/athletes/{enrollment}/update-paces', [App\Http\Controllers\Coach\AthleteController::class, 'updatePaces'])->name('athletes.update-paces');
    ...
    Route::middleware('role:eo')->prefix('eo')->name('eo.')->group(function () {
        ...
        Route::post('events/{event}/remind-whatsapp-custom', [App\Http\Controllers\EO\EventController::class, 'sendCustomWaReminderBulk'])->name('events.remind-whatsapp-custom');
