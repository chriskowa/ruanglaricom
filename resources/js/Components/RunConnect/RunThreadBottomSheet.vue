...
watch(activeTab, (newTab) => {
    ...
    if (newTab === 'info') {
        // ... (removed map.value.resize() since there is no map.value)
    }
    if (newTab === 'gpx') {
        setTimeout(() => {
...
watch(() => props.thread, (newThread) => {
    ...
    if (newThread) {
        ...
        // DO NOT call renderGpxMap here because the GPX tab is hidden (v-show).
        // It will be called by the activeTab watcher when the user switches to 'gpx'.
    }
});
...
const uploadGpx = async (event) => {
    ...
    try {
        ...
        setTimeout(() => {
            renderGpxMap();
        }, 300); // give DOM time to create container from v-if
    } catch (err) {
        ...
        >
            <div v-if="isShareOpen" class="mt-5 p-4 border border-blue-500/20 dark:border-blue-500/25 bg-blue-500/5 dark:bg-blue-500/5 rounded-xl space-y-3">
                ...
                <div class="flex items-center justify-center gap-4">
                    ...
                    <button 
                        @click="shareWhatsApp"
                        class="w-10 h-10 flex items-center justify-center bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-full transition-all cursor-pointer shadow-sm hover:scale-110"
                    ...
                    <button 
                        @click="shareTelegram"
                        class="w-10 h-10 flex items-center justify-center bg-[#0088cc] hover:bg-[#0077b5] text-white rounded-full transition-all cursor-pointer shadow-sm hover:scale-110"
                    ...
                    <button 
                        @click="copyShareLink"
                        class="w-10 h-10 flex items-center justify-center rounded-full transition-all cursor-pointer shadow-sm hover:scale-110 border"
                        :class="shareCopied 
                            ? 'bg-green-500 text-white border-green-500' 
                            : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 border-slate-200 dark:border-slate-700 hover:bg-slate-200 dark:hover:bg-slate-700'"
                        :title="shareCopied ? 'Link tersalin!' : 'Salin Link'"
                    >
                        <svg v-if="!shareCopied" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
