...
<div class="min-h-screen pt-20 pb-10 px-4 md:px-8 relative overflow-hidden font-sans" x-data="couponForm()">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        <svg class="w-4 h-4 mx-2 text-slate-600" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                        <span class="text-sm font-medium text-white">Buat Baru</span>
                    </div>
                </li>
            </ol>
        </nav>
        <h1 class="text-3xl font-black text-white italic tracking-tighter">
            BUAT <span class="text-yellow-400">KUPON</span> BARU
        </h1>
    </div>
    ...
    <div class="bg-card/50 backdrop-blur-md border border-slate-700/50 rounded-2xl overflow-hidden relative z-10 p-6 md:p-8 max-w-4xl">
        <form action="{{ route('eo.coupons.store') }}" method="POST" class="space-y-6">
            @csrf

            <!-- Event Selection -->
            <div>
                <label class="block text-sm font-bold text-slate-300 mb-2">Pilih Event <span class="text-red-500">*</span></label>
                <select name="event_id" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-500 transition-colors" required>
                    <option value="" disabled selected>-- Pilih Event --</option>
                    @foreach($events as $event)
                        <option value="{{ $event->id }}" {{ old('event_id') == $event->id ? 'selected' : '' }}>{{ $event->name }} ({{ $event->start_at ? $event->start_at->format('d M Y') : 'TBA' }})</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="value" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-500 transition-colors" :class="type === 'fixed' ? 'pl-10' : ''" placeholder="0" required min="0">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="min_transaction_amount" class="w-full bg-slate-900 border border-slate-700 rounded-xl pl-10 pr-4 py-3 text-white focus:outline-none focus:border-yellow-500 transition-colors" placeholder="0" min="0">
                    </div>
            ...
            <div class="border-t border-slate-700/50 pt-6 mt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="max_uses" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-500 transition-colors" placeholder="Kosongkan untuk tidak terbatas" min="1">
                    ...
                    <div>
                        ...
                        <input type="number" name="usage_limit_per_user" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-500 transition-colors" placeholder="Contoh: 1" min="1" value="1">
                        <p class="text-xs text-slate-500 mt-1">Berapa kali satu user bisa pakai kupon ini.</p>
                    </div>

                    <!-- Tanggal Mulai -->
                    <div>
                        <label class="block text-sm font-bold text-slate-300 mb-2">Tanggal Mulai</label>
                        <input type="datetime-local" name="start_at" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-500 transition-colors">
                    </div>

                    <!-- Tanggal Berakhir -->
                    <div>
                        <label class="block text-sm font-bold text-slate-300 mb-2">Tanggal Berakhir</label>
                        <input type="datetime-local" name="expires_at" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-yellow-500 transition-colors">
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-slate-700/50 pt-6 mt-6">
                ...
                <div class="space-y-4">
                    <label class="flex items-center space-x-3 cursor-pointer group">
                        <input type="checkbox" name="is_active" class="form-checkbox h-5 w-5 text-yellow-500 rounded border-slate-600 bg-slate-800 focus:ring-offset-slate-900 focus:ring-yellow-500" checked value="1">
                        <span class="text-slate-300 group-hover:text-white transition-colors">Aktifkan Kupon Ini</span>
                    </label>

                    <label class="flex items-center space-x-3 cursor-pointer group">
                        <input type="checkbox" name="is_stackable" class="form-checkbox h-5 w-5 text-yellow-500 rounded border-slate-600 bg-slate-800 focus:ring-offset-slate-900 focus:ring-yellow-500" value="1">
