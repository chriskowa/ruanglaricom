...
@section('content')
<div id="eo-dashboard-app" class="min-h-screen pt-20 pb-10 px-4 md:px-8 relative overflow-hidden font-sans">
    
    <!-- Hero Section -->
    <div class="mb-10 relative z-10" data-aos="fade-up">
        <div class="flex flex-col md:flex-row justify-between items-end gap-4">
            <div>
                <p class="text-yellow-400 font-mono text-sm tracking-widest uppercase mb-1">Event Operations Center</p>
            ...
            <div class="flex gap-3">
                ...
                <a href="#" class="px-6 py-3 rounded-xl bg-gradient-to-r from-yellow-600 to-orange-600 text-white font-black hover:scale-105 transition-all shadow-lg shadow-yellow-500/20 flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10 relative z-10">
        ...
        <div class="bg-card/50 backdrop-blur-md border border-slate-700/50 rounded-2xl p-6 hover:border-green-400/50 transition-all group">
            <div class="flex justify-between items-start mb-4">
                ...
                <span class="text-xs font-mono text-slate-500 uppercase">Total Revenue</span>
            </div>
            <h3 class="text-2xl font-bold text-white">Rp {{ number_format($totalEarnings, 0, ',', '.') }}</h3>
            <div class="mt-2 text-xs text-green-400 font-bold">From ticket sales</div>
        </div>

        <!-- My Events -->
        <div class="bg-card/50 backdrop-blur-md border border-slate-700/50 rounded-2xl p-6 hover:border-orange-400/50 transition-all group">
            <div class="flex justify-between items-start mb-4">
                <div class="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 group-hover:text-orange-400 transition-colors">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                </div>
                <span class="text-xs font-mono text-slate-500 uppercase">Total Events</span>
            </div>
            <h3 class="text-2xl font-bold text-white">0</h3>
            <div class="mt-2 text-xs text-slate-400">Events organized</div>
        </div>
        ...
        <div class="bg-card/50 backdrop-blur-md border border-slate-700/50 rounded-2xl p-6 hover:border-cyan-400/50 transition-all group relative overflow-hidden">
            <div class="absolute right-0 bottom-0 opacity-10 group-hover:opacity-20 transition-opacity">
                <svg class="w-24 h-24 text-cyan-400" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>
            </div>
            <div class="flex justify-between items-start mb-4">
                ...
                <span class="text-xs font-mono text-slate-500 uppercase">Total Participants</span>
            </div>
            <h3 class="text-2xl font-bold text-white">0</h3>
            <div class="mt-2 text-xs text-slate-400">Registered runners</div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <!-- Upcoming Events (Mockup) -->
        <div class="lg:col-span-2 bg-card/30 border border-slate-700 rounded-2xl p-6">
            <h3 class="text-lg font-bold text-white mb-6">Upcoming Events</h3>
            <div class="h-64 flex items-center justify-center border-2 border-dashed border-slate-700 rounded-xl bg-slate-800/50">
                <div class="text-center">
                    <svg class="w-12 h-12 text-slate-600 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                    <p class="text-slate-500 text-sm">No upcoming events scheduled</p>
                    <a href="{{ route('eo.events.create') }}" class="text-yellow-400 hover:text-yellow-300 text-sm font-bold mt-2 inline-block">Schedule one now &rarr;</a>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            <div class="bg-gradient-to-br from-yellow-900/50 to-slate-900 border border-yellow-500/30 rounded-2xl p-6 relative overflow-hidden">
                <div class="absolute top-0 right-0 -mt-4 -mr-4 w-32 h-32 bg-yellow-500/20 rounded-full blur-2xl"></div>
                <h3 class="text-white font-bold text-lg mb-2">Event Manager</h3>
                <p class="text-slate-400 text-sm mb-6">Manage registrations, check-ins, and race results efficiently.</p>
                <a href="{{ route('eo.events.index') }}" class="block w-full py-3 bg-yellow-600 hover:bg-yellow-500 text-white font-bold text-center rounded-xl transition-colors shadow-lg shadow-yellow-500/25">
                    Manage Events
                </a>
            </div>
            ...
            <div class="bg-card/50 border border-slate-700 rounded-2xl p-6">
                ...
                <div class="space-y-2">
                    ...
                    <a href="#" class="flex items-center justify-between p-3 rounded-xl bg-slate-800 hover:bg-slate-700 transition-colors group">
                        ...
                        <svg class="w-4 h-4 text-slate-500 group-hover:text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4h2v-4zM6 8V4h2v4H6zm0 8h2v4H6v-4zm6-4v-4h2v4h-2z" /></svg>
                    </a>
