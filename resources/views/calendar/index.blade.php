                    ...
                    <div id="share-container" class="fixed -left-[9999px] top-0 w-[600px] bg-[#0f172a] p-8 text-white">
                         <div class="bg-gradient-to-r from-slate-800 to-slate-900 border border-slate-700 rounded-2xl p-6 shadow-2xl relative overflow-hidden">
                            ...
                            <div class="flex items-center gap-6 mb-8 relative z-10">
                                <!-- Medal Shape Profile -->
                                <div class="w-24 h-24 shrink-0 drop-shadow-2xl">
                                    <svg viewBox="0 0 124 124" class="w-full h-full overflow-visible">
                                        <defs>
                                            <linearGradient id="medalGradient" x1="0" y1="0" x2="1" y2="1">
    ...
    <script>
        ...
        createApp({
            ...
            methods: {
                ...
                this.posterData.bgImage = null; // Reset image

                try {
                    // 1. Fetch Detail Activity untuk dapat foto HD
                    // Summary activity di list tidak punya URL foto HD, harus fetch detail
                    ...
                    if (detail.photos && detail.photos.primary && detail.photos.primary.urls) {
                        // Ambil ukuran 600px jika ada, atau fallback ke 100/default
                        bgImage = detail.photos.primary.urls['600'] || detail.photos.primary.urls['100'];
                    } 
                    // Opsional: Jika tidak ada foto, bisa pakai peta Google Static Map atau Mapbox jika punya API Key
                    // Disini kita biarkan null agar fallback ke Gradient CSS
                    ...
                    this.posterData = {
                        ...
                        pace: this.calculatePace(activity.moving_time, activity.distance).split(' ')[0], // Ambil angkanya saja
                    ...
                    // 4. Tunggu DOM Render (NextTick tidak cukup karena gambar perlu load)
                    await this.$nextTick();
                    
                    // Tunggu sebentar agar gambar terload (hacky but works for simple implementation)
                    if(bgImage) {
                    ...
                    const canvas = await html2canvas(container, {
                        ...
                        scale: 2, // High Res
                        useCORS: true, // PENTING untuk gambar dari Strava
                        allowTaint: true,
                        logging: false
                    });
                    ...
                    link.download = `Ruanglari-${activity.name.replace(/\s+/g, '-').toLowerCase()}.png`;
