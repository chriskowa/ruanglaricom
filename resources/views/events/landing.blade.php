...
<div class="min-h-screen pt-24 pb-16 px-4 md:px-8 bg-dark relative overflow-hidden font-sans">
    ...
    <!-- Hero Section -->
    <div class="max-w-7xl mx-auto mb-10 relative z-10" data-aos="fade-down">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <!-- Hero Text -->
            <div class="lg:col-span-7 space-y-5 text-center lg:text-left">
                <div class="inline-flex items-center gap-2 px-2.5 py-1 rounded-sm bg-neon/10 text-neon text-xs font-mono font-bold uppercase tracking-wider border border-neon/20">
                    <span>Ruang Lari</span>
                    <span class="text-slate-600">/</span>
                    <span>Indonesia</span>
                </div>
                <h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-white tracking-tight uppercase leading-[1.08]">
                    Jadwal Lari <span class="text-neon">2026</span> Indonesia
                </h1>
                <div class="text-slate-300 text-sm sm:text-base space-y-3 leading-relaxed max-w-2xl mx-auto lg:mx-0">
                    <p>
                        Temukan jadwal lari 2026 di Indonesia dalam satu kalender lengkap. Ruang Lari menyajikan informasi event lari terbaru, mulai dari fun run, 5K, 10K, half marathon, marathon, trail run, ultra run, hingga virtual run. Setiap event dilengkapi dengan tanggal pelaksanaan, lokasi, kategori jarak, status pendaftaran, dan tautan pendaftaran resmi.
                    </p>
                    <p class="text-xs text-slate-400">
                        Punya event lari sendiri? Anda dapat memilih fitur <strong class="text-slate-200">Submit Event Lari Gratis</strong> atau membuat <strong class="text-slate-200">Registration Page Event Lari</strong> otomatis tanpa biaya awal.
                    </p>
                </div>
                
                <div class="flex flex-col sm:flex-row justify-center lg:justify-start gap-3 pt-2">
                    <button type="button" id="btn-open-submit-event" class="px-6 py-3 rounded bg-neon text-dark font-extrabold text-xs uppercase tracking-wider hover:bg-lime-300 transition-all inline-flex items-center justify-center gap-2 shadow-sm">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4" /></svg>
                        Submit Event Lari Gratis
                    </button>
                    <a href="{{ route('eo.landing') }}" class="px-6 py-3 rounded bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs uppercase tracking-wider transition-all inline-flex items-center justify-center gap-1.5 border border-slate-800">
            ...
            <div class="lg:col-span-5 relative group" data-aos="zoom-in" data-aos-delay="100">
                @if(isset($featuredEvents) && $featuredEvents->isNotEmpty())
                    <!-- Slider Container -->
                    <div 
                        class="relative bg-slate-900 border border-slate-800 rounded-md overflow-hidden shadow-2xl"
                        x-data="{ 
                            activeIndex: 0, 
                    ...
                    >
                        ...
                        <div class="flex transition-transform duration-700 ease-out" :style="`transform: translateX(-${activeIndex * 100}%)`">
                            @foreach($featuredEvents as $event)
                                <a href="{{ $event->public_url }}" class="block w-full flex-shrink-0 relative aspect-video overflow-hidden group/slide">
                                    ...
                                    <!-- Content Overlay -->
                                    <div class="absolute bottom-0 inset-x-0 p-4 sm:p-5 flex flex-col justify-end bg-gradient-to-t from-slate-950 via-slate-950/80 to-transparent">
                                        <!-- Badges -->
                                        <div class="flex items-center justify-between mb-1.5">
                                            <span class="px-2 py-0.5 text-[9px] font-extrabold tracking-wider text-dark bg-neon rounded-sm uppercase">
                                        ...
                                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-slate-300 text-xs mt-1">
                                            ...
                                            <div class="flex items-center gap-1 truncate max-w-[200px] text-slate-300">
                                                <svg class="w-3.5 h-3.5 text-neon flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        @if($event->distances->isNotEmpty())
                                            <div class="flex flex-wrap gap-1 mt-2">
                                                @foreach($event->distances as $distance)
                                                    <span class="px-2 py-0.5 rounded-sm bg-slate-950 border border-slate-800 text-[9px] font-mono font-bold text-slate-300 uppercase tracking-tight">
                                                        {{ $distance->name }}
                                                    </span>
                                                @endforeach
                                            </div>
                        ...
                        <button 
                            ...
                            class="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-sm bg-slate-950/80 hover:bg-neon hover:text-dark text-white border border-slate-800 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100 focus:opacity-100 z-20 focus:outline-none"
                        ...
                        <button 
                            ...
                            class="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-sm bg-slate-950/80 hover:bg-neon hover:text-dark text-white border border-slate-800 flex items-center justify-center transition-all opacity-0 group-hover:opacity-100 focus:opacity-100 z-20 focus:outline-none"
                        ...
                        <div class="absolute bottom-4 right-4 flex gap-1.5 z-20">
                            <template x-for="(event, idx) in total" :key="idx">
                                <button 
                                    ...
                                    class="h-1.5 rounded-sm transition-all duration-300 focus:outline-none"
                                    :class="activeIndex === idx ? 'w-5 bg-neon' : 'w-2 bg-slate-700 hover:bg-slate-500'"
                ...
                @else
                    ...
                    <div class="relative bg-slate-900 border border-slate-800 rounded-md overflow-hidden shadow-2xl">
                        <img 
                            ...
                            fetchpriority="high"
                        >
                        <div class="absolute bottom-0 inset-x-0 bg-slate-950 border-t border-slate-800 p-4 flex justify-between items-center">
                            <div class="text-left">
                                <p class="text-[10px] font-mono font-bold text-neon uppercase tracking-widest">Update Berkala</p>
                                <p class="text-xs font-black text-white tracking-tight uppercase">Kalender Lari Indonesia</p>
                            </div>
                            <div class="px-2.5 py-0.5 rounded-sm bg-slate-900 border border-slate-800 text-[9px] font-mono font-bold text-slate-300 uppercase">
                                2026 Edition
                            </div>
                        </div>
                    </div>
    ...
    <div class="max-w-7xl mx-auto mb-8 relative z-10" data-aos="fade-up">
        <div class="bg-slate-900 border border-slate-800 rounded-md p-6 sm:p-7 shadow-2xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6">
            <div class="space-y-2 text-center md:text-left max-w-3xl">
                <div class="inline-flex items-center gap-2 px-2.5 py-1 rounded-sm bg-neon/10 text-neon text-xs font-mono font-bold uppercase tracking-wider border border-neon/20">
                    Event Organizer &amp; Komunitas Lari
                </div>
                <h3 class="text-xl sm:text-2xl font-black text-white tracking-tight uppercase">
                    Punya Event Lari? Publikasikan &amp; Buat Registration Page Gratis!
                </h3>
                <p class="text-slate-300 text-xs sm:text-sm leading-relaxed">
                    Jangkau puluhan ribu pelari di seluruh Indonesia. Dapatkan publikasi event lari gratis, landing page pendaftaran otomatis, Notifikasi WA, E-Ticket QR, &amp; pembayaran online tanpa biaya awal.
                </p>
            </div>
            
            <div class="flex flex-col sm:flex-row gap-2.5 shrink-0 w-full md:w-auto">
                <a href="{{ route('eo.landing') }}" class="px-6 py-3 rounded bg-neon hover:bg-lime-300 text-dark font-extrabold text-xs uppercase tracking-wider transition-all text-center">
                    Buat Registration Page
                </a>
                <button type="button" onclick="document.getElementById('btn-open-submit-event').click()" class="px-6 py-3 rounded bg-slate-950 hover:bg-slate-800 text-white font-bold text-xs uppercase tracking-wider transition-all border border-slate-800 text-center">
                    Submit Event Gratis
                </button>
            </div>
        </div>
    ...
    <div id="events-explorer-map-section" class="max-w-7xl mx-auto mb-8 relative z-10" data-aos="fade-up">
        <div class="bg-slate-900 border border-slate-800 rounded-md overflow-hidden shadow-2xl transition-all duration-300">
            <!-- Header bar with Minimize/Expand toggle -->
            <div class="px-4 sm:px-5 py-3.5 bg-slate-950 border-b border-slate-800 flex items-center justify-between gap-3 cursor-pointer select-none" id="btn-toggle-events-map">
                <div class="flex items-center gap-3">
                    <span class="w-2.5 h-2.5 rounded-full bg-neon animate-pulse shrink-0"></span>
                    <div>
                        <div class="flex items-center gap-2 flex-wrap">
                            <h2 class="text-xs sm:text-sm font-black text-white tracking-tight uppercase">Peta Sebaran Event Lari Indonesia</h2>
                            <span id="events-map-count" class="px-2 py-0.5 rounded-sm bg-neon/10 text-neon border border-neon/30 text-[11px] font-mono font-bold">
                ...
                <div class="flex items-center gap-1.5 sm:gap-2 shrink-0" onclick="event.stopPropagation()">
                    ...
                    <div class="relative inline-block z-50" id="events-map-layer-dropdown-wrap">
                        <button type="button" id="btn-toggle-events-map-layer" onclick="toggleEventsMapLayerMenu()" class="px-2.5 sm:px-3 py-1.5 rounded bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-200 hover:text-white text-xs font-bold transition flex items-center gap-1.5 cursor-pointer">
                        ...
                        <div id="events-map-layer-menu" class="hidden absolute right-0 top-full mt-1.5 bg-slate-950 border border-slate-800 rounded shadow-2xl py-1 z-[99999] min-w-[150px] text-xs divide-y divide-slate-800">
                            <button type="button" onclick="setEventsMapLayer('voyager')" class="w-full px-3.5 py-2 text-left text-slate-200 hover:bg-slate-900 hover:text-white flex items-center justify-between cursor-pointer transition">
                                <span>Voyager</span>
                                <span class="events-layer-check-voyager text-neon text-[11px] font-bold"><i class="fa-solid fa-check"></i></span>
                            </button>
                            <button type="button" onclick="setEventsMapLayer('osm')" class="w-full px-3.5 py-2 text-left text-slate-200 hover:bg-slate-900 hover:text-white flex items-center justify-between cursor-pointer transition">
                            ...
                            <button type="button" onclick="setEventsMapLayer('dark')" class="w-full px-3.5 py-2 text-left text-slate-200 hover:bg-slate-900 hover:text-white flex items-center justify-between cursor-pointer transition">
                            ...
                            <button type="button" onclick="setEventsMapLayer('satellite')" class="w-full px-3.5 py-2 text-left text-slate-200 hover:bg-slate-900 hover:text-white flex items-center justify-between cursor-pointer transition">
                    ...
                    <button type="button" id="btn-events-map-locate-me" class="px-2.5 sm:px-3 py-1.5 rounded bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-200 hover:text-white text-xs font-bold transition flex items-center gap-1.5 cursor-pointer" title="Deteksi Lokasi Saya">
                        <i class="fa-solid fa-location-crosshairs text-[11px] text-neon"></i>
                        <span class="hidden sm:inline">Lokasi Saya</span>
                    </button>
                    <button type="button" id="btn-events-map-recenter" class="px-2.5 sm:px-3 py-1.5 rounded bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-200 hover:text-white text-xs font-bold transition flex items-center gap-1.5 cursor-pointer" title="Pusatkan Peta ke Seluruh Event">
                        <i class="fa-solid fa-expand text-[11px]"></i>
                        <span class="hidden sm:inline">Pusatkan</span>
                    </button>
                    <button type="button" id="btn-events-map-minimize-toggle" class="px-2.5 sm:px-3 py-1.5 rounded bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-200 hover:text-white text-xs font-bold transition flex items-center gap-1.5 cursor-pointer" title="Sembunyikan / Tampilkan Peta">
                        <span id="label-events-map-toggle" class="text-xs font-bold hidden xs:inline">Sembunyikan</span>
            ...
            <div id="events-map-collapse-wrap" class="relative transition-all duration-300">
                <div id="events-explorer-map" class="w-full h-[360px] sm:h-[420px] md:h-[480px] z-0 bg-slate-950"></div>

                <!-- Interactive Map Legend & Quick Type Filter Overlay -->
                <div id="events-map-type-filter-overlay" class="absolute bottom-3 left-3 z-[400] bg-slate-950 border border-slate-800 rounded p-1.5 text-xs text-slate-300 shadow-2xl pointer-events-auto flex items-center gap-1.5 overflow-x-auto max-w-[calc(100%-24px)] no-scrollbar">
                    <span class="text-[10px] font-mono text-slate-400 font-bold uppercase tracking-wider px-1 shrink-0">Filter:</span>
                    <button type="button" onclick="setEventsMapTypeFilter('')" class="btn-event-map-pill px-2.5 py-1 rounded-sm text-xs font-extrabold transition cursor-pointer border bg-neon text-dark border-neon shrink-0" data-map-type="">
                        Semua
                    </button>
                    @if(isset($raceTypes) && $raceTypes->isNotEmpty())
                        @foreach($raceTypes as $rType)
                            ...
                            @endphp
                            <button type="button" onclick="setEventsMapTypeFilter('{{ $rType->id }}')" class="btn-event-map-pill px-2.5 py-1 rounded-sm text-xs font-bold transition cursor-pointer border bg-slate-900 border-slate-800 text-slate-300 hover:border-slate-700 hover:text-white shrink-0 flex items-center gap-1.5" data-map-type="{{ $rType->id }}">
    ...
    <!-- Filter Section -->
    <div class="max-w-7xl mx-auto mb-6 relative z-10" data-aos="fade-up" data-aos-delay="100">
        <div class="bg-slate-900 border border-slate-800 rounded-md p-4 sm:p-5 shadow-xl">
            <form id="filter-form" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3.5">
                ...
                <div class="lg:col-span-1">
                    <div class="flex justify-between items-end mb-1">
                        <label class="block text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Cari Event</label>
                    ...
                    <div class="relative">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama event atau lokasi..." class="w-full bg-slate-950 border border-slate-800 rounded px-3.5 py-2.5 pl-9 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-neon transition-colors">
                ...
                <div class="mobile-filter-item hidden md:block">
                    <label class="block text-xs font-mono font-bold text-slate-300 uppercase tracking-wider mb-1">Waktu</label>
                    <div class="flex gap-2">
                        ...
                        @endphp
                        <select name="month" class="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-neon transition-colors">
                            <option value="">Bulan</option>
                            @foreach(range(1, 12) as $m)
                                <option value="{{ $m }}" @selected($selectedMonth == $m)>{{ date('F', mktime(0, 0, 0, $m, 1)) }}</option>
                            @endforeach
                        </select>
                        <select name="year" class="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-neon transition-colors">
                ...
                <!-- City -->
                <div class="mobile-filter-item hidden md:block">
                    <label class="block text-xs font-mono font-bold text-slate-300 uppercase tracking-wider mb-1">Lokasi Kota</label>
                    <select name="city_id" class="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-neon transition-colors">
                ...
                <!-- Race Type -->
                <div class="mobile-filter-item hidden md:block">
                    <label class="block text-xs font-mono font-bold text-slate-300 uppercase tracking-wider mb-1">Jenis Lomba</label>
                    <select name="race_type_id" class="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-neon transition-colors">
                ...
                <!-- Distance -->
                <div class="mobile-filter-item hidden md:block">
                    <label class="block text-xs font-mono font-bold text-slate-300 uppercase tracking-wider mb-1">Kategori Jarak</label>
                    <select name="race_distance_id" class="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-neon transition-colors">
    ...
    <div class="max-w-7xl mx-auto mb-6 relative z-10 overflow-x-auto no-scrollbar py-1">
        <div class="flex gap-2 min-w-max pb-1">
            <button type="button" class="quick-filter-btn px-4 py-2 rounded text-xs font-extrabold uppercase tracking-wider border transition-all bg-neon text-dark border-neon shadow-sm shadow-neon/10" data-filter-type="distance" data-value="">
                Semua Jarak
            </button>
            @foreach($raceDistances as $distance)
                <button type="button" class="quick-filter-btn px-4 py-2 rounded text-xs font-bold uppercase tracking-wider border transition-all bg-slate-900 border-slate-800 text-slate-300 hover:border-slate-700 hover:text-white" data-filter-type="distance" data-value="{{ $distance->id }}">
                    {{ $distance->name }}
                </button>
            @endforeach
            
            <div class="h-5 w-px bg-slate-800 self-center mx-1.5"></div>
            
            <button type="button" class="quick-filter-btn px-4 py-2 rounded text-xs font-bold uppercase tracking-wider border transition-all bg-slate-900 border-slate-800 text-slate-300 hover:border-slate-700 hover:text-white" data-filter-type="type" data-value="">
                Semua Jenis
            </button>
            @foreach($raceTypes as $type)
                <button type="button" class="quick-filter-btn px-4 py-2 rounded text-xs font-bold uppercase tracking-wider border transition-all bg-slate-900 border-slate-800 text-slate-300 hover:border-slate-700 hover:text-white" data-filter-type="type" data-value="{{ $type->id }}">
                    {{ $type->name }}
                </button>
            @endforeach
        </div>
    ...
    <div class="max-w-7xl mx-auto relative z-10">
        ...
        <div id="events-container" class="space-y-3.5">
        ...
        <div id="loading-indicator" class="hidden py-12 text-center">
            ...
            <p class="mt-2 text-slate-300 text-xs font-mono">Memuat jadwal...</p>
        </div>
    </div>

    <!-- SEO Content Sections -->
    <div class="max-w-7xl mx-auto mt-16 pt-12 border-t border-slate-800 relative z-10 space-y-12">
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5">
                ...
                @foreach($months as $num => $monthName)
                    ...
                    @endphp
                    <a href="{{ route('events.index') }}?month={{ $mVal }}" class="p-3.5 bg-slate-900 border border-slate-800 hover:border-neon/50 rounded text-center transition group">
                        <h3 class="font-bold text-slate-200 group-hover:text-neon text-xs sm:text-sm uppercase tracking-wide transition">Jadwal Lari {{ $monthName }} 2026</h3>
                    </a>
                @endforeach
            </div>
        </div>
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2.5">
                ...
                @foreach($seoCities as $slug => $cityName)
                    <a href="/event-lari-di-{{ $slug }}" class="p-3.5 bg-slate-900 border border-slate-800 hover:border-neon/50 rounded text-center transition group">
                        <h3 class="font-bold text-slate-200 group-hover:text-neon text-xs sm:text-sm uppercase tracking-wide transition">
                            @if($slug == 'surabaya')
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                ...
                @foreach($seoCategories as $slug => $label)
                    <a href="/jadwal-{{ $slug }}" class="p-3.5 bg-slate-900 border border-slate-800 hover:border-neon/50 rounded text-center transition group">
                        <h3 class="font-bold text-slate-200 group-hover:text-neon text-xs sm:text-sm uppercase tracking-wide transition">{{ $label }}</h3>
                    </a>
                @endforeach
            </div>
        </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-md p-6 sm:p-7 space-y-3">
            <h2 class="text-xl sm:text-2xl font-black text-white tracking-tight uppercase">
                Cara Memilih Event Lari yang Tepat
            </h2>
            <div class="text-slate-300 space-y-3 leading-relaxed text-sm">
        ...
        <div class="space-y-4">
            ...
            <div class="space-y-3" x-data="{ active: null }">
                ...
                @foreach($faqs as $i => $faq)
                    <div class="border border-slate-800 rounded bg-slate-900 overflow-hidden">
                        <button @click="active = active === {{ $i }} ? null : {{ $i }}" class="w-full px-5 py-4 text-left flex justify-between items-center hover:bg-slate-800/60 transition">
                            <span class="font-bold text-white text-sm sm:text-base">{{ $faq['q'] }}</span>
                            <svg class="w-4 h-4 text-neon transition-transform shrink-0 ml-2" :class="active === {{ $i }} ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7" />
                            </svg>
                        </button>
                        <div x-show="active === {{ $i }}" x-collapse x-cloak class="px-5 pb-4 pt-1 text-slate-300 border-t border-slate-800/80 leading-relaxed text-xs sm:text-sm">
                            <p>{{ $faq['a'] }}</p>
                        </div>
                    </div>
                @endforeach
...
<div id="submit-event-modal" class="fixed inset-0 z-[9999] hidden overflow-y-auto">
    <div class="fixed inset-0 bg-slate-950/85"></div>
    <div class="relative min-h-screen w-full flex items-center justify-center p-3 sm:p-6">
        <div class="w-full max-w-5xl bg-slate-900 border border-slate-800 rounded-md shadow-2xl overflow-hidden flex flex-col my-auto max-h-[92vh] text-slate-200">
            <!-- Modal Header -->
            <div class="px-4 sm:px-8 py-4 sm:py-5 border-b border-slate-800 bg-slate-950 flex items-center justify-between shrink-0">
                <div class="flex items-center gap-3.5">
                    <div>
                        ...
                        <h3 class="text-base sm:text-xl font-black text-white tracking-tight uppercase">Ajukan Event Lari Baru</h3>
                    </div>
                </div>
                <button type="button" id="btn-close-submit-event" class="w-8 h-8 rounded-sm bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white flex items-center justify-center transition-colors">
            ...
            <form id="submit-event-form" class="flex-1 overflow-y-auto px-4 sm:px-8 py-5 sm:py-6 space-y-5 custom-scrollbar">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="bg-slate-950 border border-slate-800 rounded p-4 sm:p-5 space-y-3.5">
                            <div class="flex items-center gap-2 pb-2 border-b border-slate-800">
                                <i class="fas fa-info-circle text-neon text-xs"></i>
                                <h4 class="text-xs font-mono font-bold uppercase tracking-wider text-slate-300">Informasi Utama Event</h4>
                            </div>

                            <div class="space-y-1.5">
                                <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Nama Event <span class="text-neon">*</span></label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400 text-xs">
                                        <i class="fas fa-flag"></i>
                                    </div>
                                    <input type="text" name="event_name" id="submit_event_name" class="w-full bg-slate-900 border border-slate-800 rounded pl-9 pr-4 py-2.5 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-neon transition-all" placeholder="Contoh: Jakarta City Run 2026" required>
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div class="space-y-1.5">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Tanggal Event <span class="text-neon">*</span></label>
                                    <input type="date" name="event_date" id="submit_event_date" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-neon transition-all" required>
                                </div>
                                <div class="space-y-1.5">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Jam Mulai</label>
                                    <input type="time" value="05:00" name="start_time" id="submit_event_time" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-neon transition-all">
                                </div>
                            </div>

                            <div class="space-y-1.5">
                                <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Jenis Lomba</label>
                                <select name="race_type_id" id="submit_event_race_type_id" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-neon transition-all">
                        ...
                        <div class="bg-slate-950 border border-slate-800 rounded p-4 sm:p-5 space-y-3">
                            <div class="flex items-center justify-between pb-2 border-b border-slate-800">
                                <div class="flex items-center gap-2">
                                    <i class="fas fa-image text-neon text-xs"></i>
                                    <h4 class="text-xs font-mono font-bold uppercase tracking-wider text-slate-300">Banner Event</h4>
                                </div>
                                <span class="text-[10px] font-mono text-slate-400">Maks. 2MB (Landscape)</span>
                            </div>

                            <div id="banner-dropzone" class="border-2 border-dashed border-slate-800 hover:border-neon bg-slate-900 rounded p-5 text-center cursor-pointer transition-all duration-300 group relative">
                                ...
                                <div id="banner-dropzone-default">
                                    <div class="w-10 h-10 rounded bg-slate-950 border border-slate-800 text-neon flex items-center justify-center mx-auto mb-2.5 shadow-sm">
                                        <i class="fas fa-cloud-upload-alt text-base"></i>
                                    </div>
                                    <p class="text-xs sm:text-sm font-bold text-white">Drag &amp; drop banner event di sini</p>
                                    <p class="text-xs text-slate-300 mt-0.5">atau <span class="text-neon font-semibold underline">klik untuk menelusuri file</span></p>
                                </div>

                                <div id="banner-dropzone-preview" class="hidden">
                                    <img id="banner-preview-img" src="" alt="Banner Preview" class="max-h-36 rounded mx-auto border border-slate-800 object-cover shadow-lg">
                                    <span id="banner-filename" class="text-xs font-mono text-neon block mt-2 font-bold truncate"></span>
                                    <button type="button" id="btn-remove-banner" class="mt-2 px-3 py-1 rounded bg-red-950/60 border border-red-500/30 text-red-300 hover:bg-red-900/60 text-xs font-bold transition-all inline-flex items-center gap-1.5">
                        ...
                        <div class="bg-slate-950 border border-slate-800 rounded p-4 sm:p-5 space-y-3">
                            <div class="flex items-center justify-between pb-2 border-b border-slate-800">
                                <div class="flex items-center gap-2">
                                    <i class="fas fa-ruler-horizontal text-neon text-xs"></i>
                                    <h4 class="text-xs font-mono font-bold uppercase tracking-wider text-slate-300">Kategori Jarak Lomba</h4>
                                </div>
                                <span class="text-[10px] font-mono text-slate-400">Pilih satu / lebih</span>
                            </div>

                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                @foreach($raceDistances as $distance)
                                    <label class="cursor-pointer flex items-center gap-2 px-3 py-2 rounded-sm border border-slate-800 bg-slate-900 hover:border-slate-700 text-xs font-bold text-slate-300 transition-all select-none has-[:checked]:border-neon/60 has-[:checked]:bg-neon/10 has-[:checked]:text-neon">
                                        <input type="checkbox" name="race_distance_ids[]" value="{{ $distance->id }}" class="hidden race-distance-cb">
                                        <span class="w-3.5 h-3.5 rounded-sm border border-slate-700 flex items-center justify-center text-[9px] font-black text-dark check-box transition-all">
                                            <i class="fas fa-check opacity-0 check-mark"></i>
                                        </span>
                                        <span class="truncate font-mono">{{ $distance->name }}</span>
                                    </label>
                                @endforeach
                            </div>

                            <div class="pt-1.5">
                                <label class="text-[11px] font-mono font-bold text-slate-300 uppercase tracking-wider block mb-1">Tambah Jarak Custom (jika belum ada)</label>
                                <input type="text" name="custom_distances" id="submit_event_custom_distances" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-neon" placeholder="Contoh: 7K, 100K, 50 mil (pisahkan koma)">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="bg-slate-950 border border-slate-800 rounded p-4 sm:p-5 space-y-3">
                            <div class="flex items-center justify-between pb-2 border-b border-slate-800">
                                <div class="flex items-center gap-2">
                                    <i class="fas fa-map-marked-alt text-neon text-xs"></i>
                                    <h4 class="text-xs font-mono font-bold uppercase tracking-wider text-slate-300">Lokasi Event</h4>
                                </div>
                                <button type="button" id="btn-geolocation" class="px-2.5 py-1 rounded bg-slate-900 hover:bg-slate-800 text-neon border border-neon/30 text-xs font-bold transition-all inline-flex items-center gap-1.5 shadow-sm">
                                    <i class="fas fa-crosshairs text-[10px]"></i> Lokasi Saya
                                </button>
                            </div>
                            ...
                            <div class="relative">
                                <div class="relative">
                                    ...
                                    <input type="text" id="submit_map_search_input" class="w-full bg-slate-900 border border-slate-800 rounded pl-9 pr-9 py-2 text-xs text-white placeholder-slate-400 focus:outline-none focus:border-neon transition-all" placeholder="Cari nama tempat, venue, atau alamat..." autocomplete="off">
                                    <button type="button" id="btn-clear-map-search" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 hover:text-white text-xs hidden" title="Hapus pencarian">
                                        <i class="fas fa-times"></i>
                                    </button>
                                </div>
                                <!-- Autocomplete Results Dropdown -->
                                <div id="map-search-results" class="absolute left-0 right-0 top-full mt-1 bg-slate-950 border border-slate-800 rounded shadow-2xl z-50 max-h-52 overflow-y-auto hidden divide-y divide-slate-800"></div>
                            </div>

                            <!-- Leaflet Map Container -->
                            <div class="relative">
                                <div id="event-map" class="w-full h-40 rounded border border-slate-800 bg-slate-900 z-0"></div>
                                <div id="map-geocoding-status" class="absolute bottom-2 left-2 bg-slate-950 text-[10px] font-mono text-neon px-2 py-0.5 rounded-sm border border-slate-800 z-10 empty:hidden"></div>
                            </div>
                            ...
                            <div class="space-y-3">
                                <div class="space-y-1.5">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Kota / Kabupaten</label>
                                    <select name="city_id" id="submit_event_city_id" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-neon transition-all">
                                ...
                                <div class="space-y-1.5">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Nama Lokasi / Venue <span class="text-neon">*</span></label>
                                    <input type="text" name="location_name" id="submit_event_location" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2.5 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-neon transition-all" placeholder="Contoh: Gelora Bung Karno" required>
                                </div>

                                <div class="space-y-1.5">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Alamat Lengkap</label>
                                    <input type="text" name="location_address" id="submit_event_address" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2.5 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-neon transition-all" placeholder="Alamat / titik kumpul">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-slate-950 border border-slate-800 rounded p-4 sm:p-5 space-y-3">
                            <div class="flex items-center gap-2 pb-2 border-b border-slate-800">
                                <i class="fas fa-link text-neon text-xs"></i>
                                <h4 class="text-xs font-mono font-bold uppercase tracking-wider text-slate-300">Tautan &amp; Media Sosial</h4>
                            </div>

                            <div class="space-y-3">
                                <div class="space-y-1.5">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Link Pendaftaran Resmi</label>
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400 text-xs font-mono">
                                            <i class="fas fa-link"></i>
                                        </div>
                                        <input type="text" inputmode="url" name="registration_link" id="submit_event_registration_link" class="w-full bg-slate-900 border border-slate-800 rounded pl-9 pr-4 py-2 text-xs sm:text-sm text-white focus:outline-none focus:border-neon transition-all placeholder-slate-400" placeholder="https://... atau website pendaftaran">
                                    </div>
                                    <p class="text-[11px] text-slate-400">Otomatis ditambahkan https:// jika belum ada.</p>
                                </div>

                                <div class="space-y-1.5">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Link Media Sosial / Instagram</label>
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400 text-xs">
                                            <i class="fab fa-instagram"></i>
                                        </div>
                                        <input type="text" inputmode="url" name="social_media_link" id="submit_event_social_media_link" class="w-full bg-slate-900 border border-slate-800 rounded pl-9 pr-4 py-2 text-xs sm:text-sm text-white focus:outline-none focus:border-neon transition-all placeholder-slate-400" placeholder="https://instagram.com/... atau @username">
                                    </div>
                                    <p class="text-[11px] text-slate-400">Bisa isi username (@akun) atau link Instagram/sosmed.</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-slate-950 border border-slate-800 rounded p-4 sm:p-5 space-y-3.5">
                            <div class="flex items-center gap-2 pb-2 border-b border-slate-800">
                                <i class="fas fa-user-shield text-neon text-xs"></i>
                                <h4 class="text-xs font-mono font-bold uppercase tracking-wider text-slate-300">Kontak &amp; Verifikasi Email</h4>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div class="space-y-1.5">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Nama EO / Penyelenggara</label>
                                    <input type="text" name="organizer_name" id="submit_event_organizer_name" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-neon transition-all" placeholder="Komunitas / EO">
                                </div>
                                <div class="space-y-1.5">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Kontak Penyelenggara</label>
                                    <input type="text" name="organizer_contact" id="submit_event_organizer_contact" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-neon transition-all" placeholder="WA / Email Kontak">
                                </div>
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div class="space-y-1.5">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Nama Kamu</label>
                                    <input type="text" name="contributor_name" id="submit_event_contributor_name" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-neon transition-all" placeholder="Nama Pengaju">
                                </div>
                                <div class="space-y-1.5">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Email Kamu <span class="text-neon">*</span></label>
                                    <input type="email" name="contributor_email" id="submit_event_contributor_email" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-neon transition-all" placeholder="email@kamu.com" required>
                                </div>
                            </div>

                            <div class="space-y-1.5">
                                <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider">Catatan Tambahan</label>
                                <textarea name="notes" id="submit_event_notes" rows="2" class="w-full bg-slate-900 border border-slate-800 rounded px-3.5 py-2 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-neon transition-all" placeholder="Info kuota, syarat pendaftaran, dll"></textarea>
                            </div>

                            <!-- OTP Box -->
                            <div class="p-3.5 bg-slate-900 rounded border border-slate-800 space-y-2.5">
                                <div class="flex items-center justify-between gap-2">
                                    <label class="text-xs font-mono font-bold text-slate-300 uppercase tracking-wider block">Verifikasi OTP Email <span class="text-neon">*</span></label>
                                    <span class="text-[10px] text-slate-400 font-mono whitespace-nowrap">Kode 6 digit</span>
                                </div>
                                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                                    <div class="relative flex-1 min-w-0">
                                        <input type="text" inputmode="numeric" maxlength="6" name="otp_code" id="submit_event_otp_code" class="w-full bg-slate-950 border border-slate-800 rounded px-3.5 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-neon font-mono tracking-widest text-center placeholder-slate-400 placeholder:tracking-normal placeholder:font-sans transition-all" placeholder="Kode OTP (6 digit)">
                                    </div>
                                    <button type="button" id="btn-submit-event-send-otp" class="w-full sm:w-auto px-4 py-2.5 rounded bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold text-xs uppercase tracking-wider transition-all whitespace-nowrap text-center flex items-center justify-center shrink-0 min-h-[40px] cursor-pointer">
                                        Kirim OTP
                                    </button>
                                </div>
                            </div>
            ...
            <!-- Modal Footer -->
            <div class="px-4 sm:px-8 py-3.5 sm:py-4 border-t border-slate-800 bg-slate-950 flex flex-col sm:flex-row justify-between items-center gap-3 shrink-0">
                <div class="text-xs text-slate-400 flex items-center gap-1.5">
                    <i class="fas fa-shield-alt text-neon"></i>
                    <span>Verifikasi OTP wajib sebelum pengajuan event.</span>
                </div>
                <div class="flex items-center gap-2.5 w-full sm:w-auto">
                    <button type="button" id="btn-submit-event-cancel" class="flex-1 sm:flex-initial px-5 py-2.5 rounded bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white font-bold text-xs uppercase tracking-wider transition">Batal</button>
                    <button type="button" id="btn-submit-event-submit" class="flex-1 sm:flex-initial px-6 py-2.5 rounded bg-neon text-dark font-extrabold text-xs uppercase tracking-wider hover:bg-lime-300 transition shadow-sm">Submit Event</button>
                </div>
            </div>
        </div>
    </div>
        ...
        eventsList.forEach(event => {
            ...
            if (Array.isArray(event.distances) && event.distances.length > 0) {
                distanceBadgesHtml = '<div class="flex flex-wrap gap-1 mt-2">' + 
                    event.distances.map(d => `<span class="px-1.5 py-0.5 rounded-sm bg-slate-950 border border-slate-800 text-[9px] font-mono font-bold text-slate-300 uppercase">${d}</span>`).join('') +
                    '</div>';
            }
            ...
            const popupHtml = `
                <div class="events-custom-leaflet-popup font-sans text-xs">
                    <div class="relative aspect-video overflow-hidden bg-slate-950">
                        ...
                        <span class="absolute top-2 left-2 px-2 py-0.5 text-[9px] font-mono font-bold uppercase rounded-sm bg-slate-950 text-neon border border-neon/30">
                            ${event.race_type || 'Event Lari'}
                        </span>
                    </div>
                    <div class="p-3.5 space-y-2">
                        <h4 class="font-black text-white text-sm tracking-tight leading-tight line-clamp-2 hover:text-neon transition uppercase">
                        ...
                        ${distanceBadgesHtml}
                        <div class="pt-2">
                            <a href="${event.url}" class="w-full py-2 px-3 rounded bg-neon hover:bg-lime-300 text-dark font-extrabold text-xs uppercase tracking-wider text-center block transition">
                                Lihat Detail Event
                            </a>
                        </div>
                    </div>
    ...
    function setEventsMapTypeFilter(typeId) {
        document.querySelectorAll('.btn-event-map-pill').forEach(btn => {
            ...
            if (bType === String(typeId)) {
                btn.className = 'btn-event-map-pill px-2.5 py-1 rounded-sm text-xs font-extrabold transition cursor-pointer border bg-neon text-dark border-neon shrink-0 flex items-center gap-1.5';
            } else {
                btn.className = 'btn-event-map-pill px-2.5 py-1 rounded-sm text-xs font-bold transition cursor-pointer border bg-slate-900 border-slate-800 text-slate-300 hover:border-slate-700 hover:text-white shrink-0 flex items-center gap-1.5';
            }
        });
        ...
        document.querySelectorAll('.quick-filter-btn[data-filter-type="type"]').forEach(b => {
            if (b.getAttribute('data-value') === String(typeId)) {
                b.className = "quick-filter-btn px-4 py-2 rounded text-xs font-extrabold uppercase tracking-wider border transition-all bg-neon text-dark border-neon shadow-sm shadow-neon/10";
            } else {
                b.className = "quick-filter-btn px-4 py-2 rounded text-xs font-bold uppercase tracking-wider border transition-all bg-slate-900 border-slate-800 text-slate-300 hover:border-slate-700 hover:text-white";
            }
        });
                ...
                if (type === 'distance') {
                    ...
                    document.querySelectorAll('.btn-event-map-pill').forEach(mp => {
                        if (mp.getAttribute('data-map-type') === String(value)) {
                            mp.className = 'btn-event-map-pill px-2.5 py-1 rounded-sm text-xs font-extrabold transition cursor-pointer border bg-neon text-dark border-neon shrink-0 flex items-center gap-1.5';
                        } else {
                            mp.className = 'btn-event-map-pill px-2.5 py-1 rounded-sm text-xs font-bold transition cursor-pointer border bg-slate-900 border-slate-800 text-slate-300 hover:border-slate-700 hover:text-white shrink-0 flex items-center gap-1.5';
                        }
                    });
                }
                
                this.className = "quick-filter-btn px-4 py-2 rounded text-xs font-extrabold uppercase tracking-wider border transition-all bg-neon text-dark border-neon shadow-sm shadow-neon/10";
                fetchEvents();
            });
        });
...
<script>
    (function () {
        ...
        function showAlert(type, msg) {
            ...
            alertBox.innerHTML = '<div class="'+cls+' rounded-sm p-3 text-xs font-bold flex items-center gap-2"><i class="fas '+(type === 'success' ? 'fa-check-circle' : 'fa-exclamation-circle')+' text-sm"></i><span>'+String(msg || '')+'</span></div>';
