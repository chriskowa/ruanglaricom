    ...
    <div class="absolute inset-0 flex items-end sm:items-center justify-center p-4">
        <div class="w-full sm:max-w-lg rounded-2xl shadow-2xl {{ $modalPanelClass }}">
            <div class="p-5 sm:p-6">
                ...
                <div id="rl-moota-view-instructions" class="mt-5 grid grid-cols-1 gap-3">
                        ...
                        <div class="flex items-center justify-between gap-3">
                            <div>
                                ...
                                <div class="mt-1 flex items-center gap-2">
                                    <div class="text-2xl font-black text-slate-900" id="rl-moota-total">Rp0</div>
                                    <button type="button" id="rl-moota-copy-total" class="p-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-600 transition group relative" title="Salin Nominal">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                                        <span class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-[10px] text-white bg-black rounded opacity-0 group-hover:opacity-100 transition pointer-events-none whitespace-nowrap">Salin</span>
                                    </button>
                                </div>
                            </div>
                    ...
                    <div class="rounded-xl border border-slate-200 p-4">
                        ...
                        <div class="mt-3 hidden" id="rl-moota-actions">
                            <button type="button" id="rl-moota-wa" class="px-4 py-3 rounded-2xl bg-green-500 hover:bg-green-600 text-white font-bold flex items-center gap-2 transition">
                    ...
                    <div class="rounded-xl border border-slate-200 p-4">
                        ...
                        <div class="mt-3 text-xs text-slate-800 font-bold leading-relaxed">
                ...
                <div id="rl-moota-view-success" class="hidden mt-10 mb-6 text-center">
                    ...
                    <h3 class="text-xl font-black text-slate-900 mb-2">Terima Kasih</h3>
                    <p class="text-sm text-slate-600">
                        Jika pembayaran berhasil, sistem akan mengirimkan notifikasi tiket melalui email.
                    </p>
                </div>

                <div class="mt-5 flex items-center justify-end gap-3">
                    <button type="button" id="rl-moota-confirm-paid" class="px-5 py-3 rounded-2xl font-bold bg-slate-900 text-white hover:bg-slate-800">Saya Sudah Bayar</button>
...
<script>
    (function () {
        ...
        const elCopyTotal = document.getElementById('rl-moota-copy-total');
        ...
        const elWa = document.getElementById('rl-moota-wa');
        ...
        function openModal(payload) {
            ...
            // Reset view
            if (elViewInstructions) elViewInstructions.classList.remove('hidden');
            if (elViewSuccess) elViewSuccess.classList.add('hidden');
            if (elConfirmPaid) elConfirmPaid.classList.remove('hidden');
            if (elTitle) elTitle.textContent = 'Instruksi Pembayaran';
            if (elSubtitle) elSubtitle.textContent = 'Transfer tepat sesuai total agar terverifikasi otomatis.';
        ...
        if (elConfirmPaid) {
            elConfirmPaid.addEventListener('click', function() {
                if (elViewInstructions) elViewInstructions.classList.add('hidden');
                if (elViewSuccess) elViewSuccess.classList.remove('hidden');
                elConfirmPaid.classList.add('hidden');
                
                // Update header
                if (elTitle) elTitle.textContent = 'Menunggu Verifikasi';
                if (elSubtitle) elSubtitle.textContent = 'Terima kasih atas konfirmasi Anda.';
            });
        }

        if (elWa) {
            elWa.addEventListener('click', function () {
                const name = current.name || 'Peserta';
                const regId = elPublicRef.textContent || '-';
                const msg = `Halo, saya ${name} dengan ID Registrasi ${regId} ingin cek status pembayaran.`;
                const url = `https://wa.me/6281958662550?text=${encodeURIComponent(msg)}`;
        ...
        if (elCopyTotal) {
            elCopyTotal.addEventListener('click', function() {
                if (current.finalAmount) {
                    const text = String(current.finalAmount);
        ...
        function showCopySuccess() {
            const originalHtml = elCopyTotal.innerHTML;
            elCopyTotal.innerHTML = '<svg class="w-4 h-4 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg><span class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-[10px] text-white bg-black rounded transition pointer-events-none whitespace-nowrap">Disalin!</span>';
            setTimeout(() => {
                elCopyTotal.innerHTML = originalHtml;
            }, 2000);
        }
