                    ...
                    <tr class="hover:bg-slate-700/20 transition-colors group">
                        <td class="px-6 py-4 cursor-pointer" @click="openModal({{ $user->id }})">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full bg-slate-700 flex items-center justify-center text-slate-300 font-bold overflow-hidden relative flex-shrink-0">
                                    <img src="{{ $user->avatar_url }}" 
                                        alt="{{ $user->name }}" 
                                        class="w-full h-full object-cover"
                                        onerror="this.onerror=null; this.src='{{ asset('images/default-male.svg') }}';">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                @if(auth()->id() !== $user->id)
                                    <!-- Copy Login Token Link -->
                                    <button type="button" 
                                        class="p-2 rounded-lg hover:bg-slate-700 text-amber-400 hover:text-amber-300 transition-colors" 
                                        title="Salin Link Login Token (Akses {{ $user->name }})"
                                        @click="copyLoginToken({{ $user->id }}, '{{ addslashes($user->name) }}')">
                                    ...
                                    <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline">
                                        ...
                                        <button type="submit" class="p-2 rounded-lg hover:bg-slate-700 text-slate-400 hover:text-red-400 transition-colors"
                                            title="Delete User"
                                            onclick="return confirm('Delete {{ $user->name }}? This will permanently remove the user and related data.');">
                                ...
                                <!-- Send WhatsApp -->
                                <button type="button" 
                                    @click="openWhatsAppModal('{{ $user->id }}', '{{ addslashes($user->name) }}', '{{ $user->phone }}')" 
                                    class="p-2 rounded-lg hover:bg-slate-700 text-slate-400 hover:text-green-400 transition-colors" 
