...
<div id="admin-strength-app" class="min-h-screen pt-20 pb-12 px-4 md:px-8 relative overflow-hidden font-sans">
    ...
    <!-- Filter & Search Bar -->
    <div class="card-dark p-4 rounded-2xl border border-slate-800 mb-6 relative z-10">
        <form method="GET" action="{{ route('admin.strength-exercises.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div>
                ...
                <select name="category" onchange="this.form.submit()" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs font-bold text-white focus:border-brand-500 outline-none">
            ...
            <div class="md:col-span-2">
                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Pencarian</label>
                <div class="relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama gerakan, alat, atau otot..." class="w-full bg-slate-900 border border-slate-700 rounded-xl pl-9 pr-3 py-2 text-xs text-white focus:border-brand-500 outline-none">
    ...
    <div class="card-dark rounded-2xl border border-slate-800 overflow-hidden shadow-xl relative z-10">
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs border-collapse">
                <thead>
                    <tr class="bg-slate-900/80 text-slate-400 text-[10px] uppercase tracking-wider border-b border-slate-800">
                    ...
                    @forelse($exercises as $ex)
                        <tr class="hover:bg-slate-800/30 transition">
                            ...
                            <td class="py-3.5 px-4">
                                @php
                                    $catLabels = [
                                        'legs_lower_body' => ['Legs / Lower Body', 'bg-blue-500/10 text-blue-400 border-blue-500/20'],
                                        'core' => ['Core', 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'],
                                        'full_body' => ['Full Body', 'bg-purple-500/10 text-purple-400 border-purple-500/20'],
                                        'upper_body' => ['Upper Body', 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20'],
                                    ];
                                    $c = $catLabels[$ex->category] ?? [$ex->category, 'bg-slate-800 text-slate-300 border-slate-700'];
                                @endphp
                                <span class="px-2 py-0.5 rounded-md border text-[10px] font-bold uppercase tracking-wider {{ $c[1] }}">
                                    {{ $c[0] }}
                                </span>
                            </td>
                            ...
                            <td class="py-3.5 px-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button onclick="openEditModal({{ json_encode($ex) }})" class="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition">
    ...
    <div id="exerciseFormModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
        ...
        <div class="relative z-10 max-w-xl mx-auto my-10 p-6 card-dark border border-slate-800 rounded-2xl shadow-2xl">
            <div class="flex justify-between items-center pb-3 border-b border-slate-800 mb-4">
                ...
                <button onclick="closeFormModal()" class="text-slate-400 hover:text-white text-base">✕</button>
            </div>
            ...
            <form id="exerciseForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="label-text">Nama Gerakan</label>
                        <input type="text" id="inputName" name="name" required class="input-field font-bold" placeholder="Contoh: Barbell Squat">
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="label-text">Kategori</label>
                            <select id="selectCategory" name="category" required class="input-field cursor-pointer font-bold">
                        ...
                        <div>
                            <label class="label-text">Peralatan (Equipment)</label>
                            <input type="text" id="inputEquipment" name="equipment" class="input-field font-bold" placeholder="Bodyweight, Dumbbell, Barbell">
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="label-text">Default Sets</label>
                            <input type="text" id="inputSets" name="default_sets" class="input-field font-bold" placeholder="3">
                        </div>
                        <div>
                            <label class="label-text">Default Reps / Durasi</label>
                            <input type="text" id="inputReps" name="default_reps" class="input-field font-bold" placeholder="10-12 reps">
                        </div>
                    </div>

                    <div>
                        <label class="label-text">Target Otot</label>
                        <input type="text" id="inputMuscles" name="target_muscles" class="input-field" placeholder="Contoh: Quad, Glutes, Core">
                    </div>

                    <div>
                        <label class="label-text">Media Visual (GIF / Video MP4 / Gambar)</label>
                        <div class="grid grid-cols-3 gap-2 mb-2">
                            <select id="selectMediaType" name="media_type" class="input-field cursor-pointer font-bold text-xs">
                                <option value="gif">GIF Animasi</option>
                                <option value="video">Video MP4/WebM</option>
                                <option value="image">Foto WebP/PNG</option>
                                <option value="url">External Media URL</option>
                            </select>
                            <input type="text" id="inputMediaUrl" name="media_url" class="input-field col-span-2 text-xs" placeholder="URL Media External (Opsional)">
                        </div>
                        <input type="file" name="media_file" class="block w-full text-xs text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-extrabold file:bg-slate-800 file:text-brand-400 hover:file:bg-slate-700 cursor-pointer">
                    </div>

                    <div>
                        <label class="label-text">Instruksi Form & Panduan Langkah</label>
                        <textarea id="inputInstructions" name="instructions" rows="3" class="input-field text-xs" placeholder="Berikan petunjuk posisi tubuh, instruksi gerak, dan tips keamanan..."></textarea>
                    </div>

                    <div class="flex justify-end gap-3 pt-3 border-t border-slate-800">
                        <button type="button" onclick="closeFormModal()" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold rounded-xl transition">Batal</button>
                        <button type="submit" class="px-5 py-2 bg-brand-500 hover:bg-brand-600 text-dark text-xs font-black uppercase tracking-wider rounded-xl transition shadow-lg shadow-brand-500/20">Simpan Gerakan</button>
                    </div>
                </div>
            </form>
        </div>
