...
@section('title', 'Admin - Email Report')
...
<div class="min-h-screen pt-20 pb-10 px-4 md:px-8 relative overflow-hidden font-sans">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <span class="ml-1 text-sm font-medium text-white md:ml-2">Email Report</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
            <div>
                <h1 class="text-3xl md:text-4xl font-black text-white italic tracking-tighter">
                    EMAIL <span class="text-yellow-400">REPORT</span>
                </h1>
                <p class="text-slate-400 mt-2">Manajemen email report yang dikirim (filter: tanggal, event, status).</p>
            </div>

            <div class="flex gap-2">
                <a href="{{ route('admin.email-reports.export', request()->query()) }}" class="px-4 py-2 rounded-xl bg-slate-800 border border-slate-700 text-slate-300 hover:text-white hover:bg-slate-700 transition flex items-center gap-2 text-sm font-bold">
                    <span class="w-4 text-slate-300"><i class="fas fa-file-csv"></i></span>
                    Export CSV
                </a>
                <a href="{{ route('admin.email-reports.print', request()->query()) }}" target="_blank" class="px-4 py-2 rounded-xl bg-slate-800 border border-slate-700 text-slate-300 hover:text-white hover:bg-slate-700 transition flex items-center gap-2 text-sm font-bold">
                    <span class="w-4 text-slate-300"><i class="fas fa-file-pdf"></i></span>
                    Export PDF
                </a>
            </div>
        </div>
    ...
    <div class="bg-card/50 backdrop-blur-md border border-slate-700/50 rounded-2xl p-6 md:p-8 relative z-10">
        <form method="GET" action="{{ route('admin.email-reports.index') }}" class="flex flex-col md:flex-row gap-3 items-end">
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="text-slate-400 border-b border-slate-700">
                        ...
                        <th class="text-left py-3 pr-4">EO</th>
                ...
                <tbody class="text-slate-200">
                    @foreach($deliveries as $d)
                        <tr class="border-b border-slate-800/60">
                            ...
                            <td class="py-3 pr-4">{{ $d->eoUser?->name }}</td>
                            ...
                            <td class="py-3 pr-4 whitespace-nowrap">{{ $d->sent_at?->format('Y-m-d H:i') }}</td>
                            <td class="py-3 pr-4 text-slate-400">
                                @if($d->status === 'failed')
                                    <div class="text-red-400">{{ $d->failure_code }}</div>
                                    <div class="text-xs">{{ \Illuminate\Support\Str::limit($d->failure_message, 90) }}</div>
                                @endif
                            </td>
                        </tr>
                    @endforeach
                    @if($deliveries->count() === 0)
                        <tr>
                            <td colspan="9" class="py-6 text-center text-slate-500">Tidak ada data.</td>
                        </tr>
                    @endif
                </tbody>
            </table>
