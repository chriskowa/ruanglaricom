...
<div class="min-h-screen pt-20 pb-10 px-4 md:px-8 font-sans">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <h1 class="text-3xl font-black text-white">Edit Popup</h1>
        </div>
    ...
    <form method="POST" action="{{ route('admin.popups.update', $popup) }}" id="popup-form" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        @csrf
        @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <div class="space-y-1 text-sm">
                            <label class="flex items-center gap-2"><input type="checkbox" class="rules-device" value="desktop"> Desktop</label>
                            <label class="flex items-center gap-2"><input type="checkbox" class="rules-device" value="mobile"> Mobile</label>
                            <label class="flex items-center gap-2"><input type="checkbox" class="rules-device" value="tablet"> Tablet</label>
                        </div>
                    </div>
                    <div>
                        <div class="text-xs text-slate-400 mb-1">Segments</div>
                        <div class="space-y-1 text-sm">
                            <label class="flex items-center gap-2"><input type="checkbox" class="rules-segment" value="guest"> Guest</label>
                            <label class="flex items-center gap-2"><input type="checkbox" class="rules-segment" value="new"> New</label>
                            <label class="flex items-center gap-2"><input type="checkbox" class="rules-segment" value="returning"> Returning</label>
                            <label class="flex items-center gap-2"><input type="checkbox" class="rules-segment" value="premium"> Premium</label>
                            <label class="flex items-center gap-2"><input type="checkbox" class="rules-segment" value="member"> Member</label>
                        </div>
                    </div>
                    <div>
            ...
            <div class="bg-slate-900/60 border border-slate-700 rounded-2xl p-5 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input id="set-overlay" class="w-full px-4 py-2 rounded-xl bg-slate-900 border border-slate-700 text-white">
                    </div>
                    <div>
                        <div class="text-xs text-slate-400 mb-1">Background</div>
                        <input id="set-background" class="w-full px-4 py-2 rounded-xl bg-slate-900 border border-slate-700 text-white">
                    </div>
                    <div>
                        <div class="text-xs text-slate-400 mb-1">Text Color</div>
                        <input id="set-text" class="w-full px-4 py-2 rounded-xl bg-slate-900 border border-slate-700 text-white">
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <div class="text-xs text-slate-400 mb-1">Accent</div>
                        <input id="set-accent" class="w-full px-4 py-2 rounded-xl bg-slate-900 border border-slate-700 text-white">
                    </div>
                    <div class="flex items-center gap-4 text-sm">
                        <label class="flex items-center gap-2"><input type="checkbox" id="set-close-backdrop"> Close on backdrop</label>
                        <label class="flex items-center gap-2"><input type="checkbox" id="set-close-esc"> Close on ESC</label>
                    </div>
                </div>
            </div>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-slate-900/60 border border-slate-700 rounded-2xl p-5">
                <div class="text-sm font-bold text-white mb-3">Version History</div>
                <div class="space-y-2 max-h-80 overflow-y-auto">
                    @forelse($versions as $version)
                        <div class="flex items-center justify-between border border-slate-700 rounded-xl p-3">
                            <div>
                                <div class="text-sm text-white">Version {{ $version->version }}</div>
                                <div class="text-xs text-slate-400">{{ $version->created_at->format('d M Y H:i') }}</div>
                            </div>
                            <form method="POST" action="{{ route('admin.popups.versions.restore', [$popup, $version]) }}">
                                @csrf
                                <button class="px-3 py-1 rounded-lg bg-slate-800 text-xs text-white">Restore</button>
                            </form>
                        </div>
                    @empty
                        <div class="text-xs text-slate-400">No versions yet.</div>
                    @endforelse
                </div>
            </div>
        </div>
...
<script>
    ...
    const applyInitialState = () => {
        const rules = popupState.rules || {};
        const settings = popupState.settings || {};
        document.getElementById('rules-include').value = (rules.include_paths || ['/*']).join('\n');
        document.getElementById('rules-exclude').value = (rules.exclude_paths || []).join('\n');
        document.getElementById('rules-frequency').value = rules.frequency?.mode || 'session';
        document.getElementById('rules-interval').value = rules.frequency?.interval_hours || 24;
        document.getElementById('rules-cities').value = (rules.city_ids || []).join(',');
        document.getElementById('set-position').value = settings.position || 'center';
        document.getElementById('set-overlay').value = settings.overlay || 'rgba(15, 23, 42, 0.7)';
        document.getElementById('set-background').value = settings.background || '#0f172a';
        document.getElementById('set-text').value = settings.text_color || '#e2e8f0';
        document.getElementById('set-accent').value = settings.accent || '#ccff00';
        document.getElementById('set-animation').value = settings.animation || 'fade';
        document.getElementById('set-close-backdrop').checked = settings.close_on_backdrop ?? true;
        document.getElementById('set-close-esc').checked = settings.close_on_esc ?? true;
        const devices = rules.devices || ['desktop','mobile','tablet'];
        document.querySelectorAll('.rules-device').forEach((cb) => { cb.checked = devices.includes(cb.value); });
        const segments = rules.segments || ['guest','new','returning','premium','member'];
        document.querySelectorAll('.rules-segment').forEach((cb) => { cb.checked = segments.includes(cb.value); });
        if (Array.isArray(rules.time_windows)) {
            rules.time_windows.forEach((win) => {
                const row = document.createElement('div');
                row.dataset.window = '1';
                row.className = 'flex items-center gap-2';
                row.innerHTML = `
                    <input type="time" data-start value="${win.start || ''}" class="px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white">
                    <span class="text-slate-400">-</span>
                    <input type="time" data-end value="${win.end || ''}" class="px-3 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white">
