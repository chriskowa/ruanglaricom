...
<div class="min-h-screen pt-20 pb-10 px-4 md:px-8 relative overflow-hidden font-sans flex items-center justify-center">
    <div class="max-w-md w-full bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-8 text-center relative overflow-hidden">
        ...
        <div class="relative z-10">
            <h2 class="text-2xl font-black text-white italic tracking-tighter mb-2">COMPLETE BOOKING</h2>
            <p class="text-slate-400 text-sm mb-6 font-mono">Invoice: {{ $booking->invoice_number }}</p>

            <div class="bg-slate-950 rounded-xl p-5 mb-6 border border-slate-800 text-left">
                <div class="text-xs text-slate-500 uppercase tracking-widest mb-1">Pacer</div>
                <div class="text-white font-black">{{ $booking->pacer->user->name }}</div>
                <div class="text-xs text-slate-500 mt-2 grid grid-cols-2 gap-2">
                    <div>
                        <div class="uppercase tracking-widest">Race Date</div>
                        <div class="text-slate-200 font-mono">{{ $booking->race_date ? $booking->race_date->format('Y-m-d') : '-' }}</div>
                    </div>
                    <div>
                        <div class="uppercase tracking-widest">Target Pace</div>
                        <div class="text-slate-200 font-mono">{{ $booking->target_pace ?: '-' }}</div>
                    </div>
                </div>
            </div>

            <div class="bg-slate-950 rounded-xl p-6 mb-6 border border-slate-800">
                ...
                <div class="mt-3 text-xs text-slate-500">
                    Platform fee: Rp {{ number_format($booking->platform_fee_amount, 0, ',', '.') }} • Pacer payout: Rp {{ number_format($booking->pacer_amount, 0, ',', '.') }}
                </div>
            </div>
...
<script type="text/javascript">
    document.getElementById('pay-button').onclick = function(){
        snap.pay(@json($booking->snap_token), {
            onSuccess: function(result){
                window.location.href = @json(route('pacer.show', $booking->pacer->seo_slug)) + '?booking=paid';
            },
