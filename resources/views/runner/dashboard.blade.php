...
<div class="min-h-screen pt-5 pb-10 px-4 md:px-8 relative overflow-hidden font-sans bg-[#060a17]">
    ...
    <div x-show="showPhoneModal" 
        ...
        <div class="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden p-6 sm:p-7"
             x-transition:enter="transition ease-out duration-200"
             x-transition:enter-start="opacity-0 scale-95"
             x-transition:enter-end="opacity-100 scale-100">
            
            <div class="flex items-start gap-3.5 mb-4">
                <div class="w-11 h-11 rounded-xl bg-slate-800 border border-slate-700/60 text-neon flex items-center justify-center shrink-0">
                    <i class="fas fa-mobile-alt text-lg"></i>
                </div>
                <div>
                    <span class="inline-block text-[11px] font-mono font-semibold text-neon uppercase tracking-wider">Verifikasi Akun</span>
                    <h3 class="text-base sm:text-lg font-bold text-white tracking-tight mt-0.5">Nomor Handphone Wajib Diisi</h3>
                </div>
            </div>

            <p class="text-xs text-slate-300 leading-relaxed mb-5">
                Demi keamanan akun dan kelancaran pendaftaran event, coaching, serta transaksi di RuangLari, silakan masukkan nomor handphone aktif Anda.
            </p>

            <form @submit.prevent="submitPhoneNumber()">
                <div class="mb-5">
                    <label class="block text-xs font-semibold text-slate-300 mb-2">Nomor Handphone (WhatsApp)</label>
                    <div class="relative">
                        ...
                        <input type="tel" 
                               ...
                               class="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white font-medium text-sm focus:outline-none focus:border-neon transition"
                               pattern="[0-9+ ]{8,20}">
                    </div>
                    <p class="text-[11px] text-slate-400 mt-1.5">Gunakan format angka saja, minimal 8 digit.</p>
                </div>
                ...
                <div class="flex flex-col gap-2">
                    <button type="submit" 
                            ...
                            class="w-full py-3 bg-neon hover:bg-white disabled:bg-slate-800 disabled:text-slate-500 text-slate-950 font-bold text-xs rounded-xl transition text-center tracking-wider uppercase flex items-center justify-center gap-2">
        ...
        <div class="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden p-6 sm:p-7"
            ...
            <div class="flex items-start justify-between gap-3 mb-4">
                <div class="flex items-center gap-3.5">
                    ...
                    <div>
                        <span class="inline-block text-[11px] font-mono font-semibold text-neon uppercase tracking-wider">Profil Pelari</span>
                        <h3 class="text-base sm:text-lg font-bold text-white tracking-tight mt-0.5">Lengkapi Profil Anda</h3>
                    </div>
                </div>
                <button type="button" 
            ...
            <p class="text-xs text-slate-300 leading-relaxed mb-5">
                Lengkapi profil Anda untuk mengaktifkan sinkronisasi kalender latihan, analisis pace rekomendasi AI Coach, dan verifikasi slot race.
            </p>
            ...
            <div class="space-y-3 mb-6">
                <div class="flex items-center justify-between p-3.5 rounded-xl bg-slate-950 border border-slate-800">
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-lg bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-300 text-sm shrink-0">
                            <i class="fas fa-camera"></i>
                        </div>
                        <div>
                            <div class="text-xs font-semibold text-white">Foto Profil (Avatar)</div>
                            <div class="text-[11px] text-slate-400">Identitas akun pelari</div>
                        </div>
                    </div>
                    @if(auth()->user()->avatar)
                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                            <i class="fas fa-check text-[9px]"></i> Lengkap
                        </span>
                    @else
                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20">
                            <i class="fas fa-clock text-[9px]"></i> Belum Ada
                        </span>
                    @endif
                </div>
                ...
                <div class="flex items-center justify-between p-3.5 rounded-xl bg-slate-950 border border-slate-800">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="text-xs font-semibold text-white">Nomor Handphone</div>
                            <div class="text-[11px] text-slate-400">Notifikasi & pendaftaran event</div>
                        </div>
                    </div>
                    @if(auth()->user()->phone)
                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
            ...
            <div class="flex flex-col gap-2.5">
                <a href="{{ route('profile.show') }}" class="w-full py-3 bg-neon hover:bg-white text-slate-950 font-bold text-xs rounded-xl transition text-center tracking-wider uppercase shadow-md hover:shadow-neon/20">
                    Lengkapi Profil Sekarang
                </a>
                <button type="button" 
                        @click="showProfileCompletionModal = false; sessionStorage.setItem('dismiss_profile_modal', 'true')" 
                        class="w-full py-2.5 bg-slate-800/80 hover:bg-slate-800 border border-slate-700/60 text-slate-300 hover:text-white font-medium text-xs rounded-xl transition text-center">
                    Nanti Saja
                </button>
            </div>
        </div>
