<!DOCTYPE html>
<html lang="@yield('html_lang', app()->getLocale() ?: 'id')">
<head>
    <meta charset="UTF-8">
    ...
    <script>
        ...
        @if($ga)
        gtag('config', '{{ $ga }}');
        window.phGaId = @json($ga);
        @endif
        @if($gads)
        gtag('config', '{{ $gads }}');
        window.phGadsId = @json($gads);
        @endif
    </script>
    @endif
    ...
    <meta name="title" content="@yield('meta_title', $defaultTitle)">
    ...
    @if(empty($skipHeavyAssets) && (!empty($needCalendar) || (isset($isDashboard) && $isDashboard)))
