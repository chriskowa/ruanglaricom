...
<div class="min-h-screen pt-24 pb-12 px-4 md:px-8 font-sans" x-data="{
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-8">
        ...
        <a href="{{ route('marketplace.seller.products.create') }}" 
           class="w-full sm:w-auto px-5 py-2.5 rounded-md bg-neon hover:bg-white text-slate-950 font-black transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-wider shadow-sm">
            <span>+ Add Product</span>
        </a>
    </div>

    @if(session('success'))
        <div class="bg-slate-900 border border-slate-700 text-slate-200 p-4 rounded-md mb-6 flex items-center gap-3 text-sm">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-slate-900/80 border border-slate-800 p-4 rounded-lg">
            ...
            <div class="text-xl md:text-2xl font-black text-white font-mono">{{ number_format($soldProductsCount ?? 0) }}</div>
        </div>
        <div class="bg-slate-900/80 border border-slate-800 p-4 rounded-lg">
        ...
        <div x-show="activeTab === 'products'">
            ...
            <div class="flex flex-wrap items-center gap-2 mb-5">
                <a href="{{ route('marketplace.seller.products.index', ['product_filter' => 'all']) }}"
                   class="px-3.5 py-1.5 rounded-md text-xs font-bold transition-all border {{ ($productFilter ?? 'all') === 'all' ? 'bg-neon text-dark border-neon' : 'bg-slate-900 text-slate-300 border-slate-800 hover:border-slate-700' }}">
                    Semua ({{ $totalProductsCount ?? 0 }})
                </a>
                <a href="{{ route('marketplace.seller.products.index', ['product_filter' => 'active']) }}"
                   class="px-3.5 py-1.5 rounded-md text-xs font-bold transition-all border {{ ($productFilter ?? '') === 'active' ? 'bg-neon text-dark border-neon' : 'bg-slate-900 text-slate-300 border-slate-800 hover:border-slate-700' }}">
                    Aktif ({{ $activeProductsCount ?? 0 }})
                </a>
                <a href="{{ route('marketplace.seller.products.index', ['product_filter' => 'sold']) }}"
                   class="px-3.5 py-1.5 rounded-md text-xs font-bold transition-all border {{ ($productFilter ?? '') === 'sold' ? 'bg-slate-800 text-white border-slate-600' : 'bg-slate-900 text-slate-300 border-slate-800 hover:border-slate-700' }}">
                    Terjual ({{ $soldProductsCount ?? 0 }})
                </a>
                <a href="{{ route('marketplace.seller.products.index', ['product_filter' => 'archived']) }}"
                   class="px-3.5 py-1.5 rounded-md text-xs font-bold transition-all border {{ ($productFilter ?? '') === 'archived' ? 'bg-amber-500 text-dark border-amber-400' : 'bg-slate-900 text-slate-300 border-slate-800 hover:border-slate-700' }}">
                            ...
                            <tr class="hover:bg-slate-800/30 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @if($product->sale_type === 'auction' && $product->auction_end_at)
                                        <p class="text-slate-500 text-xs mt-1 font-mono">End: {{ $product->auction_end_at->format('d M H:i') }}</p>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @if($product->is_sold)
                                        <div class="space-y-1">
                                            ...
                                            @if($product->sold_channel_note)
                                                <div class="text-xs text-slate-400 italic">"{{ $product->sold_channel_note }}"</div>
                                            @endif
                                        </div>
                                ...
                                <td class="px-6 py-4 text-right text-sm">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        @if(!$product->is_sold && !$product->is_archived && $product->is_active)
                                            <button @click="boostProduct({{ $product->id }})" class="px-2.5 py-1.5 rounded-md bg-slate-950 border border-slate-800 text-slate-300 hover:text-neon hover:border-neon/40 text-xs font-bold transition-all" title="Boost ke Peringkat Teratas">
                                                Boost
                                            </button>

                                            <button @click="requestFeatured({{ $product->id }})" class="px-2.5 py-1.5 rounded-md {{ $product->isFeaturedActive() ? 'bg-neon/20 border-neon/40 text-neon' : 'bg-slate-950 border-slate-800 text-slate-300 hover:text-amber-300 hover:border-amber-400/40' }} text-xs font-bold transition-all border" title="Pasang Featured Product 3 Hari (Rp 25.000)">
                                        ...
                                        <!-- Mark Sold / Unsold -->
                                        @if($product->is_sold)
                                            <button @click="markUnsold({{ $product->id }})" class="px-2.5 py-1.5 rounded-md bg-slate-800 border border-slate-700 text-slate-300 hover:text-white text-xs font-bold transition-all" title="Batalkan Status Terjual">
                                                Batal Sold
                                            </button>
                                        @else
                                            <button @click="openSoldModal({{ json_encode($product) }})" class="px-2.5 py-1.5 rounded-md bg-slate-800 border border-slate-700 text-slate-200 hover:bg-white hover:text-slate-950 text-xs font-bold transition-all" title="Tandai Terjual (Sold)">
                                                Sold
                                            </button>
                                        @endif

                                        <!-- Archive / Unarchive Button -->
                                        <button @click="toggleArchive({{ $product->id }})" class="px-2.5 py-1.5 rounded-md border text-xs font-bold transition-all {{ $product->is_archived ? 'bg-amber-500/20 border-amber-500/40 text-amber-300 hover:bg-amber-500/30' : 'bg-slate-950 border-slate-800 text-slate-400 hover:text-amber-300 hover:border-amber-500/30' }}" title="{{ $product->is_archived ? 'Keluarkan dari Arsip' : 'Arsipkan Produk' }}">
                                            {{ $product->is_archived ? 'Unarchive' : 'Arsip' }}
                                        </button>

                                        <!-- View Detail -->
                                        <a href="{{ route('marketplace.show', $product->slug) }}" target="_blank" class="p-1.5 rounded-md bg-slate-950 border border-slate-800 text-slate-400 hover:text-neon transition-colors" title="View Detail">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                        </a>

                                        <!-- Edit -->
                                        <a href="{{ route('marketplace.seller.products.edit', $product->id) }}" class="p-1.5 rounded-md bg-slate-950 border border-slate-800 text-slate-400 hover:text-neon transition-colors" title="Edit Product">
                                        ...
                                        <form action="{{ route('marketplace.seller.products.destroy', $product->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus produk ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-1.5 rounded-md bg-slate-950 border border-slate-800 text-slate-400 hover:text-rose-400 transition-colors" title="Delete">
                ...
                <div class="block md:hidden divide-y divide-slate-800/60">
                    ...
                    <div class="p-4 space-y-4">
                        <div class="flex gap-3">
                            <div class="w-16 h-16 rounded-lg overflow-hidden border border-slate-800 bg-slate-950 flex-shrink-0 flex items-center justify-center">
                                @if($product->primaryImage)
                                    <img class="w-full h-full object-cover" src="{{ asset('storage/' . $product->primaryImage->image_path) }}" alt="" />
                                @else
                                    <svg class="w-6 h-6 text-slate-700" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                            ...
                            <div class="flex-grow min-w-0">
                                ...
                                <div class="mt-1.5 flex flex-wrap gap-1">
                                    @if($product->approval_status === 'pending')
                                        <span class="text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-amber-500/20 border border-amber-500/30 text-amber-300">Menunggu Review</span>
                                    @elseif($product->approval_status === 'rejected')
                                        <span class="text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-rose-500/20 border border-rose-500/30 text-rose-300">Ditolak</span>
                                    @endif
                                    @if($product->is_sold)
                                        <span class="text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-slate-800 border border-slate-700 text-slate-200">SOLD</span>
                                    @endif
                                    @if($product->is_archived)
                                        <span class="text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-amber-500/20 border border-amber-500/30 text-amber-300">ARCHIVED</span>
                                    @endif
                                    @if($product->isFeaturedActive())
                                        <span class="text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-neon/20 border border-neon/30 text-neon">FEATURED</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                        <div class="flex justify-between items-center pt-2 border-t border-slate-850/40">
                            ...
                            <div>
                                ...
                                <span class="text-xs text-slate-400 font-medium flex items-center justify-end gap-1 mt-0.5">
                                    {{ number_format($product->views_count ?? 0) }} Views
                                </span>
                            </div>
                        </div>
                        <div class="flex flex-wrap gap-2 pt-1">
                            @if(!$product->is_sold && !$product->is_archived && $product->is_active)
                                <button @click="boostProduct({{ $product->id }})" class="flex-1 bg-slate-950 border border-slate-800 text-neon font-bold py-2 rounded-md text-xs hover:bg-neon hover:text-dark transition-all">Boost</button>
                                <button @click="requestFeatured({{ $product->id }})" class="flex-1 bg-slate-950 border border-slate-800 text-amber-300 font-bold py-2 rounded-md text-xs hover:bg-amber-400 hover:text-dark transition-all">Featured</button>
                            @endif
                            @if($product->is_sold)
                                <button @click="markUnsold({{ $product->id }})" class="flex-1 bg-slate-800 border border-slate-700 text-slate-200 font-bold py-2 rounded-md text-xs hover:bg-slate-700 transition-all">Batal Sold</button>
                            @else
                                <button @click="openSoldModal({{ json_encode($product) }})" class="flex-1 bg-slate-800 border border-slate-700 text-slate-200 font-bold py-2 rounded-md text-xs hover:bg-white hover:text-slate-950 transition-all">Mark Sold</button>
                            @endif
                            <button @click="toggleArchive({{ $product->id }})" class="flex-1 bg-slate-950 border border-slate-800 text-amber-300 font-bold py-2 rounded-md text-xs hover:bg-amber-500 hover:text-dark transition-all">{{ $product->is_archived ? 'Unarchive' : 'Arsip' }}</button>
                            <a href="{{ route('marketplace.seller.products.edit', $product->id) }}" class="flex-1 text-center bg-slate-950 border border-slate-800 text-white font-bold py-2 rounded-md text-xs hover:bg-slate-800 transition-all">Edit</a>
                        </div>
                    </div>
                    @empty
                        ...
                        <div class="p-4 space-y-3">
                            ...
                            @if($order->status === 'paid')
                                <div class="pt-2 border-t border-slate-800 flex flex-col sm:flex-row gap-2 items-center">
                                    <input type="text" x-model="trackingNo" placeholder="Nomor Resi Pengiriman (Opsional)" class="w-full sm:flex-1 bg-slate-950 border border-slate-700 rounded-md px-3 py-2 text-xs text-white placeholder-slate-500 focus:border-neon outline-none">
                                    <button @click="processOrder({{ $order->id }}, trackingNo)" class="w-full sm:w-auto px-4 py-2 bg-neon text-dark font-black rounded-md text-xs uppercase tracking-wider hover:bg-lime-400 transition-all">
                                        Proses & Kirim
                                    </button>
                                </div>
                            @endif
        ...
        <div x-show="activeTab === 'history'" style="display: none;">
            <div class="bg-slate-900/60 border border-slate-800 rounded-lg overflow-hidden shadow-sm p-4 md:p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="p-4 border border-slate-800/80 rounded-lg bg-slate-950/20 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
    ...
    <div x-show="showSoldModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
        <div class="bg-slate-900 border border-slate-800 rounded-lg p-6 w-full max-w-md shadow-2xl relative" @click.away="showSoldModal = false">
            <div class="flex items-center justify-between mb-4 pb-3 border-b border-slate-800">
                <h3 class="text-lg font-black text-white italic tracking-tight">TANDAI PRODUK TERJUAL (SOLD)</h3>
                <button @click="showSoldModal = false" class="text-slate-400 hover:text-white">&times;</button>
            </div>

            <template x-if="selectedProduct">
                <div class="mb-4 bg-slate-950 p-3 rounded-md border border-slate-800 flex items-center gap-3">
                    <div class="w-10 h-10 rounded-md bg-slate-900 border border-slate-800 flex items-center justify-center shrink-0 overflow-hidden text-xs font-mono text-neon font-bold">
                        GEAR
                    </div>
            ...
            <form @submit.prevent="submitMarkSold" class="space-y-4">
                <div>
                    ...
                    <select x-model="soldForm.sold_channel" required class="w-full bg-slate-950 border border-slate-700 rounded-md px-4 py-2.5 text-sm text-white focus:border-neon outline-none">
                ...
                <div>
                    ...
                    <input type="text" x-model="soldForm.sold_channel_note" placeholder="Contoh: Tokopedia Store / COD GBK Senayan" class="w-full bg-slate-950 border border-slate-700 rounded-md px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-neon outline-none">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 uppercase mb-1.5">Dijual Kepada Siapa? (Nama Pembeli - Opsional)</label>
                    <input type="text" x-model="soldForm.sold_to_buyer_name" placeholder="Contoh: Budi Santoso (@budi_runner)" class="w-full bg-slate-950 border border-slate-700 rounded-md px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:border-neon outline-none">
                </div>

                <div class="pt-3 flex gap-3">
                    <button type="button" @click="showSoldModal = false" class="flex-1 py-2.5 rounded-md bg-slate-800 text-slate-300 font-bold text-xs uppercase hover:bg-slate-700 transition-all">
                        Batal
                    </button>
                    <button type="submit" class="flex-1 py-2.5 rounded-md bg-neon text-dark font-black text-xs uppercase hover:bg-white transition-all shadow-sm">
                        Simpan Status Sold
                    </button>
                </div>
            </form>
