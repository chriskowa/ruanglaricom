                    ...
                    <div class="flex-1">
                        <div class="flex items-start justify-between gap-4">
                            <div class="space-y-1">
                                ...
                                @elseif(($notification->reference_type === 'MasterGpx' || in_array($notification->type, ['gpx_submission', 'gpx_published', 'gpx_approved', 'gpx_title_suggestion'])) && $notification->reference_id)
                                    <a href="{{ auth()->user()?->role === 'admin' ? ($notification->type === 'gpx_title_suggestion' ? route('admin.master-gpx.index', ['tab' => 'suggestions']) : route('admin.master-gpx.index')) : route('gpx.show', $notification->reference_id) }}" class="notification-link text-white font-bold hover:text-neon transition" data-notification-id="{{ $notification->id }}">{{ $notification->title }}</a>
