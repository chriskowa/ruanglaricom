...
<!-- ========================================================================= -->
<div id="free-run-save-modal" class="fixed inset-0 z-[999999] bg-black/85 backdrop-blur-md flex items-center justify-center p-4 hidden select-none">
    <div class="bg-[#0F172A] border border-slate-700 rounded-2xl max-w-lg w-full p-5 sm:p-6 space-y-4 shadow-2xl text-slate-200">
        
        <!-- Modal Header -->
        <div class="flex items-center justify-between border-b border-slate-800 pb-3.5">
            <div>
                <h3 class="text-lg font-bold text-white">Ringkasan Aktivitas Lari</h3>
                <span class="text-xs text-slate-400">Selamat! Sesi lari Anda telah selesai direkam.</span>
            </div>
        ...
        <div class="bg-[#1E293B] border border-slate-700 rounded-xl p-3.5 grid grid-cols-4 divide-x divide-slate-700 text-center">
            <div>
                ...
                <span id="modal-dist" class="text-base sm:text-lg font-bold font-mono text-white mt-0.5 block">0.00 km</span>
            </div>
            <div>
                <span class="text-[10px] text-slate-400 font-mono font-medium block">Waktu</span>
                <span id="modal-time" class="text-base sm:text-lg font-bold font-mono text-white mt-0.5 block">00:00</span>
            </div>
            <div>
                <span class="text-[10px] text-slate-400 font-mono font-medium block">Avg Pace</span>
                <span id="modal-pace" class="text-base sm:text-lg font-bold font-mono text-[#FC4C02] mt-0.5 block">--:--</span>
            </div>
            <div>
                <span class="text-[10px] text-slate-400 font-mono font-medium block">Gain</span>
                <span id="modal-gain" class="text-base sm:text-lg font-bold font-mono text-white mt-0.5 block">+0m</span>
            </div>
        </div>

        @auth
            <!-- Save Activity Form -->
            <form id="free-run-form" onsubmit="submitFreeRunActivity(event)" class="space-y-4">
                <div>
                    <label class="block text-xs font-semibold text-slate-300 mb-1.5">Judul Aktivitas</label>
                    <input type="text" id="free-run-title" required class="w-full bg-[#1E293B] border border-slate-700 focus:border-[#FC4C02] rounded-lg p-2.5 text-xs text-white outline-none">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-300 mb-1.5">Catatan Lari (Opsional)</label>
                    <textarea id="free-run-notes" rows="2" placeholder="Bagaimana kondisi rute dan fisik Anda hari ini?" class="w-full bg-[#1E293B] border border-slate-700 focus:border-[#FC4C02] rounded-lg p-2.5 text-xs text-white outline-none resize-none"></textarea>
                </div>
                ...
                <button type="submit" id="btn-save-free-run" class="w-full py-3 px-4 rounded-xl bg-[#FC4C02] hover:bg-[#e04300] text-white font-bold text-xs uppercase tracking-wider transition flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-[#FC4C02]/20">
        ...
        @else
            ...
            <div class="p-4 rounded-xl bg-[#1E293B] border border-slate-700 space-y-3 text-center">
                ...
                <div class="flex items-center justify-center gap-2.5 pt-1">
                    ...
                    <button type="button" onclick="exportRecordedTrackGpxDirectly()" class="px-3.5 py-2.5 rounded-lg border border-slate-600 text-slate-300 text-xs font-semibold hover:text-white transition cursor-pointer">
                        Download GPX Saja
                    </button>
                </div>
            </div>
...
@push('scripts')
    ...
    <script>
        ...
        function handleRunGpsSuccess(pos) {
            ...
            if (gpsInd) {
                gpsInd.className = 'w-3 h-3 rounded-full bg-emerald-500 shadow-sm shrink-0';
            }
            if (statusLbl) {
            ...
            if (runAccuracyCircle) {
                ...
                runAccuracyCircle.setRadius(Math.min(accuracy, 40));
            }

            // Record Breadcrumb Point with Anti-Drift Filter
