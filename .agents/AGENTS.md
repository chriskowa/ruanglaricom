...
34. **Elevasi & Kedalaman Dark Mode (Surface Hierarchy)**: Hindari membuat seluruh tampilan hitam pekat datar tanpa kedalaman. Bangun hierarki permukaan: kanvas dasar (`bg-[#090A0E]`), kartu/kontainer (`bg-[#12161F]` atau `bg-[#18181B]`), input/elemen fokus (`bg-[#1E2430]`).
...
37. **Kontras Pembungkus Formulir & Kartu (Anti-Elemen Melayang)**: Warna `div` pembungkus form, kartu ringkasan, dan panel input DILARANG sama atau terlalu mendekati warna background kanvas utama (`bg main`). Harus memiliki elevasi permukaan yang jelas dan tegas (misal: kanvas `#090A0E`, pembungkus kartu/form `#131722` atau `#18181B` dengan border halus `border-zinc-800`) agar form dan teks terlihat berjangkar (*anchored*), kokoh, dan tidak terlihat mengambang tanpa wadah.
