...
- **Graphify Codebase Knowledge Graph & Local Brain (`.agents/skills/graphify`)**: Manfaatkan skill Graphify untuk memetakan dependensi kode (controller, model, view, migration), community clustering, dan eksekusi prompt berkecepatan tinggi. Jika `graphify-out/graph.json` tersedia di workspace, periksa relasi graph terlebih dahulu sebelum melakukan pembacaan file mentah secara berulang. Jalankan `graphify .` untuk initial build dan `graphify . --update` untuk update inkremental.
...
10. **Tipografi Berkarakter Sesuai Brand (Pakem Home: 'Inter Tight', 'Sora', sans-serif)**: Semua judul dan heading (H1, H2, H3, H4) WAJIB menggunakan font pakem resmi platform: `font-family: 'Inter Tight', 'Sora', sans-serif` dengan `font-weight: 800` dan `letter-spacing: -0.03em`. DILARANG menggunakan Oswald, Bebas Neue, atau default sans-serif untuk heading umum platform. Gunakan 'Plus Jakarta Sans' atau 'Inter' untuk body text, dan JetBrains Mono / tabular-nums untuk angka/telemetri.
...
33. **Tanpa Border Putih / Terang di Dark Mode (Anti-Wireframe Kaku)**: Dilarang keras memakai `border-white`, `border-white/*`, atau `border-slate-300` di sekeliling kontainer, kartu, span, a href, div, badge, maupun tombol dalam dark mode. Border putih membuat kontras terlalu tajam, merusak tema gelap, dan membuat mata sakit. Wajib gunakan border gelap yang menyatu dan tenang (`border-slate-800`, `border-zinc-800`, atau hex `#1e293b`). Hindari memberi border pada elemen kecil (seperti tag, badge, span) yang sudah memiliki perbedaan warna latar.
34. **Elevasi & Kedalaman Dark Mode (Surface Hierarchy)**: Hindari membuat seluruh tampilan hitam pekat datar tanpa kedalaman. Bangun hierarki permukaan dengan Tailwind standard palette: kanvas dasar (`bg-slate-950`), kartu/kontainer (`bg-slate-900`), input/elemen fokus (`bg-slate-950` atau `bg-slate-800`) dengan border gelap halus `border-slate-800`.
35. **Teks Keterangan & Span Wajib Terang/Putih di Dark Mode**: Semua teks keterangan, span, label, dan deskripsi di dark mode harus menggunakan font terang yang jelas (`text-white`, `text-zinc-100`, `text-zinc-200`, `text-zinc-300`). Dilarang memakai abu-abu gelap (`text-slate-500`, `text-zinc-600`) yang tenggelam dan sulit dibaca.
36. **Desain Bersih, Elegan, dan Memaksimalkan UX (Bukan Polos Kosong)**: Desain harus terlihat profesional, tenang, dan berkelas seperti platform atletik modern (Strava, NRC, The Athletic). Gunakan ruang bernapas (whitespace), hierarki visual yang tajam, dan aksen warna brand yang matang tanpa hiasan AI yang aneh (tanpa dot breathing, tanpa gradasi berlebih, tanpa badge bertumpuk-tumpuk).
37. **Kontras Pembungkus Formulir & Kartu (Anti-Elemen Melayang)**: Warna `div` pembungkus form, kartu ringkasan, dan panel input DILARANG sama atau terlalu mendekati warna background kanvas utama (`bg main`). Harus memiliki elevasi permukaan yang jelas dan tegas (misal: kanvas `bg-slate-950`, pembungkus kartu/form `bg-slate-900` atau `bg-slate-800` dengan border halus `border-slate-800`) agar form dan teks terlihat berjangkar (*anchored*), kokoh, dan tidak terlihat mengambang tanpa wadah.
38. **Anti-Arbitrary Tailwind Hex Classes (Dilarang Arbitrary `bg-[#...]` / `bg-[#...]/...`)**: Dilarang keras memakai arbitrary hex classes di Tailwind seperti `bg-[#090A0E]`, `bg-[#090A0E]/90`, `bg-[#12161F]`, `from-[#090A0E]`, dll. Class arbitrary ini sering tidak ter-compile pada berbagai halaman sehingga background hilang atau rusak. Wajib gunakan Tailwind standard scale (`bg-slate-950`, `bg-slate-900`, `bg-slate-800`, `border-slate-800`, `text-white`, `text-slate-200`, `text-slate-300`, `text-slate-400`) atau definisikan class CSS resmi di tag `<style>`.
...
| Kegunaan            | Class / Standar                         | JANGAN Gunakan                                 |
|---------------------|-----------------------------------------|------------------------------------------------|
| **Font Heading**    | `'Inter Tight', 'Sora', sans-serif`     | `Oswald`, `Bebas Neue`, sans-serif generik     |
