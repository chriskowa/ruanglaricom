...
class User extends Authenticatable
{
    ...
    /**
     * Get the last performance check-in / PB update date.
     */
    public function getLastPerformanceCheckinAt(): ?\Carbon\Carbon
    {
        if (isset($this->attributes['last_performance_checkin_at']) && $this->attributes['last_performance_checkin_at']) {
            try {
                return \Carbon\Carbon::parse($this->attributes['last_performance_checkin_at']);
            } catch (\Throwable $e) {}
        }

        $audit = $this->audit_history;
        if (is_array($audit) && !empty($audit['last_performance_checkin_at'])) {
            try {
                return \Carbon\Carbon::parse($audit['last_performance_checkin_at']);
            } catch (\Throwable $e) {}
        }
    ...
    /**
     * Record a completed performance check-in timestamp.
     */
    public function recordPerformanceCheckin(): void
    {
        $now = now();
        $audit = $this->audit_history ?? [];
        if (!is_array($audit)) {
        ...
        $audit['last_performance_checkin_at'] = $now->toIso8601String();
        $this->audit_history = $audit;

        if (\Illuminate\Support\Facades\Schema::hasColumn('users', 'last_performance_checkin_at')) {
            $this->last_performance_checkin_at = $now;
        }
